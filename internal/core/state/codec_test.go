...
func TestMarshalDecode_RoundTripsLinksDeterministically(t *testing.T) {
	home := filepath.Join(t.TempDir(), "home")
	snapshot := corestate.Snapshot{
		Home: home,
		Links: map[corestate.Key]corestate.LinkRecord{
			{ModuleID: "git", PlacementID: "local"}: {
				Target:          filepath.Join(home, ".config", "git", "local"),
				ResolvedTarget:  filepath.Join(home, ".config", "git", "local"),
				LinkDestination: filepath.Join(home, "dotfiles", "modules", "git", "local"),
			},
			...
			{ModuleID: "app", PlacementID: "config"}: {
				...
				LinkDestination: filepath.Join(home, "dotfiles", "modules", "app", "config"),
			},
		},
	}
	...
	if appIndex < 0 || gitIndex < 0 || appIndex >= gitIndex {
		t.Fatalf("Marshal() links are not sorted by module ID: %s", first)
	}
	...
	if configIndex < 0 || localIndex < 0 || configIndex >= localIndex {
		t.Fatalf("Marshal() links are not sorted by placement ID: %s", first)
	}
...
func TestDecode_RequiresLinksAndAcceptsExplicitEmptyArray(t *testing.T) {
	home := filepath.Join(t.TempDir(), "home")
	decoded, err := corestate.Decode(
		[]byte(fmt.Sprintf(`{"version":4,"home":%q,"links":[]}`, home)),
		home,
	)
	if err != nil {
		t.Fatalf("Decode() error = %v", err)
	}
	if decoded.Home != home || decoded.Links == nil || len(decoded.Links) != 0 {
		t.Fatalf("Decode() = %#v, want explicit empty links", decoded)
	}
	for _, document := range []string{
		fmt.Sprintf(`{"version":4,"home":%q}`, home),
		fmt.Sprintf(`{"version":4,"home":%q,"links":null}`, home),
	} {
...
func TestDecode_RejectsUnsafeOrAmbiguousDocuments(t *testing.T) {
	...
	validLink := fmt.Sprintf(
		`{"module_id":"app","placement_id":"config","target":%q,"resolved_target":%q,"link_destination":%q}`,
		target,
		target,
		linkDestination,
	)
	tests := []struct {
	...
	}{
		{
			...
			document: fmt.Sprintf(`{"version":4,"version":4,"home":%q,"links":[]}`, home),
		},
		{
			name:     "trailing JSON",
			document: fmt.Sprintf(`{"version":4,"home":%q,"links":[]}{}`, home),
		},
		{
			name:     "link null",
			document: fmt.Sprintf(`{"version":4,"home":%q,"links":[null]}`, home),
		},
		{
			name: "unknown link field",
			document: fmt.Sprintf(
				`{"version":4,"home":%q,"links":[{"module_id":"app","placement_id":"config","target":%q,"resolved_target":%q,"link_destination":%q,"extra":true}]}`,
				home,
				target,
				target,
				linkDestination,
		...
		{
			name: "v3 kind field is unknown",
			document: fmt.Sprintf(
				`{"version":4,"home":%q,"links":[{"module_id":"app","placement_id":"config","kind":"link","target":%q,"resolved_target":%q,"link_destination":%q}]}`,
				home,
				target,
				target,
				linkDestination,
		...
		},
		{
			...
			document: fmt.Sprintf(
				`{"version":4,"home":%q,"links":[{"Module_ID":"app","placement_id":"config","target":%q,"resolved_target":%q,"link_destination":%q}]}`,
				home,
				target,
				target,
				linkDestination,
		...
		},
		{
			...
			document: fmt.Sprintf(
				`{"version":4,"home":%q,"links":[{"module_id":"Bad","placement_id":"config","target":%q,"resolved_target":%q,"link_destination":%q}]}`,
				home,
				target,
				target,
				linkDestination,
		...
		},
		{
			...
			document: fmt.Sprintf(
				`{"version":4,"home":%q,"links":[{"module_id":"app","placement_id":"Bad","target":%q,"resolved_target":%q,"link_destination":%q}]}`,
				home,
				target,
				target,
				linkDestination,
		...
		},
		{
			...
			document: fmt.Sprintf(
				`{"version":4,"home":%q,"links":[{"module_id":"app","placement_id":"config","target":%q,"resolved_target":"relative","link_destination":%q}]}`,
				home,
				target,
				linkDestination,
			),
		...
		{
			name: "missing resolved target",
			document: fmt.Sprintf(
				`{"version":4,"home":%q,"links":[{"module_id":"app","placement_id":"config","target":%q,"link_destination":%q}]}`,
				home,
				target,
				linkDestination,
			),
		...
		{
			name: "null resolved target",
			document: fmt.Sprintf(
				`{"version":4,"home":%q,"links":[{"module_id":"app","placement_id":"config","target":%q,"resolved_target":null,"link_destination":%q}]}`,
				home,
				target,
				linkDestination,
			),
		...
		{
			name: "null link destination",
			document: fmt.Sprintf(
				`{"version":4,"home":%q,"links":[{"module_id":"app","placement_id":"config","target":%q,"resolved_target":%q,"link_destination":null}]}`,
				home,
				target,
				target,
			),
		...
		},
		{
			...
			document: fmt.Sprintf(
				`{"version":4,"home":%q,"links":[{"module_id":"\ud800"}]}`,
				home,
			),
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !errors.Is(err, corestate.ErrInvalid) {
				t.Fatalf("Decode() = (%#v, %v), want ErrInvalid", decoded, err)
			}
...
func TestDecode_ClassifiesVersionsBeforeLegacySchema(t *testing.T) {
	tests := []struct {
		...
		{version: "3", want: corestate.ErrLegacyVersion},
		{version: "4", want: corestate.ErrInvalid},
		{version: "5", want: corestate.ErrTooNew},
		...
		{version: `"5"`, want: corestate.ErrInvalid},
	}
	for _, test := range tests {
...
func TestMarshal_RejectsInvalidConstructedSnapshot(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "relative link destination",
			snapshot: corestate.Snapshot{
				Home: home,
				Links: map[corestate.Key]corestate.LinkRecord{
					{ModuleID: "app", PlacementID: "config"}: {
...
func TestEqualComparesHomeAndLinkOwnership(t *testing.T) {
	home := filepath.Join(t.TempDir(), "home")
	key := corestate.Key{ModuleID: "app", PlacementID: "config"}
	link := corestate.LinkRecord{
		Target:          filepath.Join(home, ".config", "app"),
		ResolvedTarget:  filepath.Join(home, ".config", "app"),
		LinkDestination: filepath.Join(home, "repo", "app"),
	}
	base := corestate.Snapshot{Home: home, Links: map[corestate.Key]corestate.LinkRecord{key: link}}
	if !corestate.Equal(base, base) {
		t.Fatal("Equal(snapshot, snapshot) = false")
	}
	if corestate.Equal(base, corestate.Snapshot{Home: home, Links: map[corestate.Key]corestate.LinkRecord{}}) {
		t.Fatal("Equal() ignored link ownership")
	}
	if corestate.Equal(base, corestate.Snapshot{Home: filepath.Dir(home), Links: base.Links}) {
		t.Fatal("Equal() ignored HOME binding")
	}
}
...
func TestDecodeRejectsInvalidUTF8ExpectedHome(t *testing.T) {
	...
	decoded, err := corestate.Decode(
		[]byte(`{"version":4,"home":"/valid","links":[]}`),
		invalidHome,
	)
	if !errors.Is(err, corestate.ErrInvalid) ||
		decoded.Home != "" ||
