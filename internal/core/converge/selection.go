...
func Initialize(
	...
	if err != nil {
		return SelectionResult{}, newFailure(
			FailureStageInput, false, RecoveryNone, nil, err,
		)
	}
	...
	if err := validateLockBoundary(environment); err != nil {
		return SelectionResult{}, newFailure(
			FailureStageInput, false, RecoveryPaths, nil, err,
		)
	}
	release, err := acquireLock(filepath.Dir(environment.LockPath), environment.LockPath)
	if err != nil {
		return SelectionResult{}, newFailure(
			FailureStageLock, false, RecoveryNone, nil, err,
		)
	}
	defer func() { err = joinReleaseFailure(err, release()) }()
