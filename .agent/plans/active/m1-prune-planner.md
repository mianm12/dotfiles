...
- [ ] 测试先行补齐 orphan plan-time resolution，并形成独立 fix commit。
- [ ] 测试先行实现 P1/P2/P3 自包含 prune actions。
- [ ] 测试先行实现 full/partial scope、完整 desired 整模块组、no-prune 与 conflict deferred。
- [ ] 运行窄测、20 次重复、race、完整 diff check 与 `make check`，保持 active/clean 等待 review。
...
尚未完成。计划保持 active，等待实现、验证和主线程安排独立 review。
