...
Linux/amd64 apply test binary 交叉编译、`git diff 34f8609...6fbef01 --check` 和
`make check BINARY=/private/tmp/dot-m1-cp4-self-traversal-r1-check/dot` 全部通过。worktree clean；
尚未完成独立完整复核、plan closure、main 集成和整个 Checkpoint 重验收。远端 macOS/Linux CI
未运行；Linux 证据仅为交叉编译。
