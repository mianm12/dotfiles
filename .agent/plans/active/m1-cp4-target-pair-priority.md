...
- [ ] 先提交能暴露 earlier ancestor / later equal 的 paths/runtime 回归。
...
| 必须成立的性质 | 验证证据 | 状态 |
|---|---|---|
| earlier ancestor 不遮蔽 later equal | paths structured error regression | 待验证 |
| 无 equal 时首个 ancestor 与 unary 顺序不变 | paths negative controls | 待验证 |
| 多 state key equal identity 始终分类为 corrupt | runtime isolated loading regression | 待验证 |
| resolution failure 保持可信前置 | paths resolution-first regression | 待验证 |
