...
- recovery mutation 在严格控制面校验后持锁但跳过 manifest/state；control-only recovery 不取锁；
  update 后进入完整 apply 时显式复用 `*lock.Ownership`。
...
recovery mutation 在控制面校验后持锁但跳过 state/manifest，以及 control-only recovery 完全
只读。用 corrupt/too-new/rendered state 证明这些入口不会被旧 state 阻塞；update 后嵌套 full
load 仍复用外层 owner，并在进入 state 依赖阶段恢复 fail closed。
...
recovery mutation 持锁且不读 state；control-only recovery 不创建 state root/lock；嵌套 full
load 的 lease 释放不解开外层锁。
...
| 必须成立的性质 | 验证证据 | 状态 |
|---|---|---|
| mutation 顺序与锁后失败释放 | operations 顺序测试 + 真实锁重取 | 待验证 |
| read-only/dry-run 零锁零写入 | 临时树快照与 lock 不存在断言 | 待验证 |
| strict manifest 实际 requirement 复核与 dev notice 数据 | compatibility 测试 | 待验证 |
| state missing/loaded/fail-closed 与路径错误分类 | 真实 state fixture 测试 | 待验证 |
| init/recovery 不被损坏 state 阻塞 | 恢复入口真实文件系统测试 | 待验证 |
| ownership nested reuse 不提前释放 | 外层/内层/竞争者生命周期测试 | 待验证 |
| 当前平台完整门禁 | `make check` | 待验证 |
| macOS/Linux 可编译 | 双平台测试二进制交叉编译 | 待验证 |
...
尚在实施。分支 base 为 `af990e80f1fe`；计划保持 active，待实现、门禁和独立复核完成后由
coordinator 按 Checkpoint 流程收口。远端 macOS/Linux CI 尚未运行。
