...
- [ ] 按 DAG 完成 state-store 与 runtime-loading 的实现、review → fix → review、closure 与 main
  集成。
...
| 必须成立的性质 | 主要证据 | 状态 |
|---|---|---|
| 配置优先级、严格解码与 config-missing policy | preflight 表驱动与零写入测试 | 待验证 |
| state 分类与任意层级重复 member | state codec/语义测试 | 待验证 |
| M1 rendered fail-closed | state 与 runtime loader 测试 | 待验证 |
| state root/file/lock 0700/0600 | state-store/process-lock 文件系统测试 | 待验证 |
| 原子失败保留旧 state | store 故障注入与并发 reader 测试 | 待验证 |
| 锁竞争、释放、崩溃恢复与嵌套复用 | 真实 helper 子进程测试 | 待验证 |
| 只读路径零写入零锁 | runtime tree snapshot 与占锁测试 | 待验证 |
| 恢复前置不被损坏 state 阻塞 | runtime policy 顺序/访问测试 | 待验证 |
...
进行中。runtime-preflight、state-v1 与 process-lock 已按 DAG 完成独立计划、语义 commits、
review → fix → review、freshness、closure 与本地 main 集成；current main 为 `2029afe`，对应
Wave 2 后完整门禁已通过。下一节点是从该 clean main 创建 state-store；其后仍需
runtime-loading、完整 Acceptance、coordinator freshness、计划迁移与最终本地 main
fast-forward-only 集成。
