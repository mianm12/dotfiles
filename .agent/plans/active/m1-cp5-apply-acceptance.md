...
- [ ] 测试先行修复 L1 EEXIST pure mismatch 及 runner/CLI 端到端分类。
- [ ] 测试先行修复 EOF confirmation Close IO error 聚合。
- [ ] 运行窄测、重复、race、完整 diff check 与隔离 cache `make check`，保持 active 等待独立复核。
