...
- [ ] 完成 target pair priority fix 的测试先行实现、完整独立复核和 FF-only main 集成。
- [ ] 从 `checkpoint_base...main` 重做完整 CP4 Acceptance，更新 Outcomes/Handoff 并再次收口。
...
| 完整解析 target-set 内 equal 优先于 ancestor，state 损坏分类不依赖 key 顺序 | paths/runtime regressions | 待验证 |
| 完整 Checkpoint 本地门禁 | checkpoint diff check + make check | 待重验收 |
...
Checkpoint 4 的既有本地完成结论已因 target pair priority P1 失效，本计划当前重新开启；以下
内容仅记录此前已完成的历史基线，不代表本轮最终 Acceptance。原始三个 Milestone 按严格 DAG
独立计划、测试先行、语义提交、
