...
- [ ] 2026-07-20：完成当前 `checkpoint_base...main` 三路 Acceptance、最终本地门禁、
  Outcomes/Handoff 更新和纯计划 closure，再 FF-only 合入 main。
...
`make check`。最终至少运行：
    ...
    make check BINARY=/private/tmp/dot-m1-cp4-acceptance-final
...
重新开启。以下内容保留首次收口时的历史结论；`eb5753d` 之后的 evidence-contract、state-topology
修复和最终 Acceptance 将在本轮 closure 中补齐，在此之前不再把本节视为当前最终结论。

Checkpoint 4 已完成本地交付。三个 Milestone 按严格 DAG 依次从有效 main 基线创建、独立计划、
测试先行、语义提交、完整复审、closure 后 fast-forward-only 合入：`feat/apply-link@f4522b0`、
`feat/apply-scaffold@061b783`、`feat/apply-runtime@0a6233d`。首轮 Acceptance 的两个有效 P2 由
`fix/m1-apply-core-acceptance@eb5753d` 修复并独立复审，随后三名未参与实现的 reviewer 对
`e9e8bac...main@eb5753d` 完整 Checkpoint 重新验收，均 GO 且无 P0–P3。

最终实现提供内部 link/scaffold executor、严格 Precond/no-clobber/原子替换、hard-link 隔离、
validated state transition，以及锁内 exact-input plan→execute→部分成功单次 persistence 链路；
Store 发布失败后由 L2/S1b 恢复，稳定重跑达到零 executor/adopt/Store。真实非 dry-run
`dot apply` 仍在 runtime 前硬拒绝，state v1、公开输出/退出码与 M2/M3 范围均未改变，也未新增
依赖。

最终本地 diff check、窄测、CLI 拒绝回归与 `make check` 在 Darwin/arm64 通过，所有任务 worktree
均 clean；worker worktree 已无 force 移除。精确最终 HEAD 未 push，远端 macOS/Linux CI 未实际
运行，因此 handoff 结论是：**本地验收通过、远端待验收**。
