...
- 分离完整 profile 的 identity/topology validation 与 scope 内 leaf observation。
...
- [ ] 增加按需 leaf 证据和语义 Precondition 的失败回归，并完成 planner/executor contract 修复。
- [ ] 增加 action classification/executor preflight 回归，删除 skip 死分支并澄清 Result 字段。
- [ ] 运行窄测、race、完整 diff check 和 `make check`，完成独立复核与计划收口。
...
release ownership 前恢复为 owned 时仍拒绝。随后拆分完整 identity join 与 scope leaf scan，移除
通用 Observation raw bytes，并让 executor 按 action 所需 leaf predicate 复核。
...
| 必须成立的性质 | 验证证据 | 状态 |
|---|---|---|
| 完整 identity/path validation 不因 partial 削弱 | planner complete-profile collision/ancestor regressions | 待验证 |
| 非 scope regular payload 不被读取 | unreadable partial target regression | 待验证 |
| S1b/L6 只依赖规范要求的 leaf 证据 | planner + executor filesystem regressions | 待验证 |
| owned migration 和 target mutation Precond 不降级 | executor symlink/identity/ancestor regressions | 待验证 |
| unsupported plan 在首个 mutation 前拒绝 | apply preflight regression | 待验证 |
| skip/conflict 只有一个职责归属 | planner/apply/executor contract tests | 待验证 |
| 既有 L/S、恢复和幂等不回退 | 五包窄测、race、完整 `make check` | 待验证 |
