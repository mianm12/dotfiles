...
- Observation: 两个直接 target resolution 的 equal、left-ancestor、right-ancestor 关系互斥；
  只有 control entry/consumed 的 cross-product 需要内部 bitset 聚合。
  Evidence: `TargetResolution.IsAncestorOf` 明确排除 equal，两个严格祖先方向在同一无环解析拓扑
  中不能同时成立；paths 现有测试覆盖三种方向和 symlink traversal。
  Impact: 对外 `TargetRelation` 使用互斥枚举，而不是暴露内部 `pathRelation` bitset；runtime
  对 equal 的分类无需含糊的 flag 组合判断。
