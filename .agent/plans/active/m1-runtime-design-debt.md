...
| 必须成立的性质 | 验证证据 | 状态 |
|---|---|---|
| production preflight 零值安全、上下文不可伪造 | runtime/CLI tests | 待验证 |
| 锁后失败不丢失会话且 release 可重试 | runtime/lock failure tests | 待验证 |
| nested/recovery/init/read-only 边界不回归 | runtime filesystem/order tests | 待验证 |
| state 提交只使用 session 可信 control path | runtime/state tests | 待验证 |
