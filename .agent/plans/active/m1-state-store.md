...
- [ ] 测试先行实现确定性 v1 编码并形成独立语义 commit。
- [ ] 测试先行实现原子 store、完整故障矩阵并形成独立语义 commit。
- [ ] 完成全部本地门禁、更新 handoff，保持计划 active 等待独立复核。
...
| 必须成立的性质 | 验证证据 | 状态 |
|---|---|---|
| state v1 确定性、严格可回读编码 | codec round-trip 与顺序测试 | 待验证 |
| 首次/替换发布只见完整新文件 | 真实文件系统 store 测试 | 待验证 |
| state root/state.json 为 0700/0600 | mode 测试 | 待验证 |
| 所有发布前失败保留旧 bytes/mode | 逐阶段故障注入矩阵 | 待验证 |
| cleanup 失败保留旧 state 并报告双重 cause | cleanup 故障测试 | 待验证 |
| 无新增依赖且双平台可编译 | tidy/module audit 与交叉编译 | 待验证 |
...
尚未实施。计划提交后先完成确定性编码，再完成原子 store 与故障矩阵；worker 最终保持本计划
active，等待独立复核与 lifecycle closure。
