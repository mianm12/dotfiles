...
实现已完成并通过 worker 本地门禁，计划保持 active 等待未参与实现的独立 reviewer。当前 commits
为 `42ce69d`（计划）、`ddc10fc`（scaffold publication/runner/state）、`1f673cb`（提交边界故障
回归）和 `3268ec8`（术语修正）。完整 diff 相对有效 base `cff70b6e` 仅包含本计划与
`internal/add` 实现/测试；Round 1 fix 为 `4f8f9d6`。未新增依赖、CLI、README、持久化格式或
ownership 变化。

本机已通过相关包普通测试、add 5 次重复、add/apply race、定向 lint、完整 diff check、独立
`GOCACHE`/`GOLANGCI_LINT_CACHE`/BINARY 的 `make check`，Darwin/Linux amd64 add test binary
交叉编译通过。Round 1 fix 后已重跑 add 5 次重复、add/apply race、完整 diff check 与隔离
`make check`。真实 Linux 主机和远端 macOS/Linux CI 未运行：本地 Darwin 验收通过、远端待验收。
