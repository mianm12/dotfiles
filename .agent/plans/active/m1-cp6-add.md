...
- [ ] 2026-07-21：Wave 1 实现以 `8d22f24`、`11c72dc` 完成并通过 worker 门禁；round 1
  完整独立复核发现两个 P1（批内 source variant/文件系统别名未整体保留、Git 继承 `GIT_*`
  可偏离 effective repo/config）和一个 P2（BatchPlan 可伪造/修改）。主 agent 验证均有效，
  原 worker 正以新测试与 fix commits 修复，之后将对完整 branch 执行 round 2。
- [ ] Wave 1：`feat/add-preflight` 独立计划、实现、复核、closure 和 main 集成。
- [ ] Wave 2：`feat/add-link` 独立计划、实现、复核、closure 和 main 集成。
- [ ] Wave 3：`feat/add-scaffold` 独立计划、实现、复核、closure 和 main 集成。
