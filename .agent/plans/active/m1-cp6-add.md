...
指引。根据用户定义的三轮 review 上限，当前暂停：`main@0ddb42a` 保留已验收的四个 Milestone，
`fix/m1-add-acceptance@ba61b30` 与本 coordinator plan 均保持 active/clean，未合入 acceptance fix、
未完成 Checkpoint Acceptance 或 coordinator closure。真实 Linux 与远端 macOS/Linux CI 未运行。
