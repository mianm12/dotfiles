...
state、CLI I/O 或规范。窄范围 race 与全仓 freshness gate 均通过；远端 macOS/Linux CI、真实 Linux
和真实私人配置未运行。计划按父任务要求保持 `active/`，worktree clean 后交给未参与实现者只读
复核与父任务集成；本分支不自行完成 plan closure。
