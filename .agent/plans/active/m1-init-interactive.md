...
- [x] 2026-07-22：Round 2 独立复核给出唯一 P2：init config-only 缺少可信 context，nested apply 又
  ...
  projection 抑制重复 context，普通 apply 保持。CLI targeted race 与 lint 通过；等待 final gate。
...
`InitSession`，测试 seam
先真实关闭再注入错误并重新取锁证明无泄漏。Round 2 的唯一 P2 已实现 targeted fix。状态：active，
等待 final gate 与 Round 3 完整独立复核。按父任务边界，本 worker 不迁移
