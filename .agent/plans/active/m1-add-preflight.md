...
- [x] 2026-07-22：fix 后两包普通/5 次重复/race、定向 lint、完整 base diff check、Darwin/Linux
  amd64 add test binary 交叉编译及独立 cache/BINARY `make check` 全部通过；等待完整 Round 2 复审。
...
实现与 Round 1 fix 完成，计划保持 active 等待完整复审。`8d22f24` 建立精确 prospective source overlay，
...
Milestone fix 门禁通过，远端待验收；Round 2 完整复审未开始，因此不迁移 completed、不声称
review-ready。
