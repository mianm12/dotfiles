...
- [ ] Wave 1：两个独立 worker 已开始并行交付，完成后依次 review、freshness 与集成
  hooks-run-once、init-config。
...
| hook 顺序/scope/cwd/env/exec-sh/stdio | planner/executor/apply/CLI tests | 待验证 |
| hook at-least-once/失败后缀/成功前缀/单次 state commit | apply/state failure tests | 待验证 |
| init strict merge/repo persistence/0600/Precond | manifest/config/runtime tests | 待验证 |
