...
- [ ] 2026-07-20：以失败回归固定 upsert × orphan 拓扑缺口，再实现 planner 前置 gate。
...
| 必须成立的性质 | 主要证据 | 状态 |
|---|---|---|
| upsert 与任一保留 orphan 不同 identity、互不为祖先 | planner relation matrix | 待验证 |
| no-prune/deferred/partial 均在 executor 前拒绝危险组合 | planner + apply integration | 待验证 |
| 拒绝时 target/state 零 mutation，重复运行可诊断 | real isolated `apply.Run` regression | 待验证 |
