...
可提交前缀都已验证；plan-only preserve 不改集合，early migration 可以解除 alias 冲突，later
migration 不能倒推修复不安全前缀。

Concrete steps：

    go test ./internal/planner -run 'TestValidateFileStateTopology'
    go test ./internal/apply -run 'TestRun_Rejects.*CandidateStateTopologyBeforeExecutor'
...
| CP4 既有 L/S、Precond、恢复与幂等不回退 | 五包窄测、race、完整 `make check` | 通过 |
...
待执行。精确 HEAD 的远端 macOS/Linux CI 未运行。
