...
- [x] 2026-07-19：以 `542e85f` 单独提交本 active ExecPlan 起点。
- [x] 2026-07-19：先增加共享 API 行为测试，确认因入口不存在而编译失败；随后完成普通、init
  与 repository-only preflight、严格持久 repo 校验、完整控制面验证和 data 副本。runtime
  窄测及 `-count=20` 均通过，未新增依赖。
- [x] 2026-07-19：version 改用 repository-only preflight；新增控制面重叠回归，并对齐 dangling
  repo symlink 的更早 fail-closed 阶段。runtime/CLI 窄测及 `-count=20` 均通过；manifest-only
  既有非法 config/state 隔离测试保持通过。
- [x] 2026-07-19：runtime/config/paths/CLI 相关测试 `-count=20` 通过；darwin/amd64 与
  linux/amd64 编译通过；隔离 Go/lint cache 后 `make check
  BINARY=/private/tmp/dot-cp2-preflight-check` 通过。
- [x] 2026-07-19：未参与实现的只读 reviewer 对 `5b099066...HEAD` 完整复核为 GO，无
  P0–P3 actionable finding；主 agent 核对后接受该结论。
- [x] 2026-07-19：review 后最终 `git diff 5b099066...HEAD --check` 与
  `make check BINARY=/private/tmp/dot-cp2-preflight-final` 通过；worktree clean，完成计划收口。
...
| 必须成立的性质 | 验证证据 | 状态 |
|---|---|---|
| 严格 machine config 与被遮蔽 repo 仍 fail closed | runtime table tests | 通过 |
| repo/profile 优先级和 config-missing 三种策略 | runtime table tests | 通过 |
| cwd 无关且控制面完整隔离 | runtime/path filesystem tests | 通过 |
| preflight 零 state/lock/文件写入 | 临时树前后快照 | 通过 |
| version 新机与错误诊断不回归 | CLI tests | 通过 |
| manifest-only 不读取 config/state | 既有 CLI isolation tests | 通过 |
| 当前平台完整门禁 | `make check` | 通过 |
| macOS/Linux 可编译 | darwin/linux amd64 编译 | 通过；Linux 未运行 |
...
- Observation: runtime 测试首次运行被 sandbox 阻止写入用户 Go build cache。
  Evidence: 默认 `go test` 报告无法打开 `~/Library/Caches/go-build`；把 `GOCACHE` 指向
  `/private/tmp/dot-cp2-preflight-gocache` 后测试正常运行。
  Impact: 后续本地门禁继续使用隔离 `GOCACHE`；这不改变代码、范围或测试语义。

- Observation: version 接入完整控制面校验后，dangling repo symlink 会在 requires 的 repo
  `Stat` 之前由控制面 identity 解析拒绝。
  Evidence: 既有 CLI 测试仍期望后续的 `inspect repository`，实际错误已经精确指向
  `resolve repository control path` 和 `ErrPathBlocked`。
  Impact: 更新回归以固定规范要求的更早 fail-closed 阶段；退出码、`requires=error` 与零写入
  行为不变。
...
Milestone 已完成本地收口。分支基线为 `5b099066`，语义 commits 为计划起点 `542e85f`、
共享 preflight `aa5a447`、CLI 复用 `154b850` 和实施证据 `f4b2a1b`。新增
`internal/runtime` 的普通、init、repository-only 三个入口；version 已复用 repository-only，
manifest-only 保持原专用路径。

本地证据包括 runtime/config/paths/CLI 相关测试 20 次重复通过，darwin/amd64 与 linux/amd64
编译通过，worker 与 review 后完整 `make check` 均通过。未参与实现的只读 reviewer 完整复核
为 GO，无 P0–P3；主 agent 复查完整 diff、错误边界与证据后未发现需修复项。实现未新增依赖，
未读取 state/manifest，未创建 lock/state，也未改变 config 或其他持久数据。Linux 仅完成编译，
远端 macOS/Linux CI 未运行；结论为“本地验收通过、远端待验收”。当前满足 Milestone
review-ready 与 lifecycle closure 条件，可由 coordinator fast-forward-only 集成到 main。
