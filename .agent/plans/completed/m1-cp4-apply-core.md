...
- [x] 2026-07-20：收口后独立重审确认一个新的 P1：共享 target-set 在较早 ancestor pair 上
  立即返回，会遮蔽同一已完整解析集合中较晚的 equal-identity pair，使 runtime 把损坏 state
  错分为 `ErrPathValidation`。main 仍为 clean `09147de`，无 DAG 外提交或额外 worktree。
- [x] 2026-07-20：按现有 Goal 授权重新开启 coordinator；确定新增串行修复节点
  `fix/m1-cp4-target-pair-priority`，不改变 state 格式、ownership、mutation 或公开 CLI 范围。
- [x] 2026-07-20：`m1-cp4-target-pair-priority@8a0f8a1` 以测试先行方式固定
  `resolution failure > equal > 首个 ancestor > unary`；两名独立 reviewer 完整复核 GO，
  freshness 与门禁通过后 FF-only 合入 main。
- [x] 2026-07-20：首次重做三路完整 Acceptance 时，执行内核两路 GO、一路仅发现 README 把
  “内部 executor 已交付”和“公开 apply 尚未接线”混为旧阶段的 P2；复用本 Goal acceptance
  branch，以 `m1-cp4-readme-apply-boundary@aa3a3f6` 修正并经两份独立复审 GO 后 FF-only 合入。
- [x] 2026-07-20：三名未参与实现的 reviewer 对修复后的
  `checkpoint_base...main@aa3a3f6` 重新完整验收，规范/数据流、数据保护/恢复、Go/测试/双平台
  三路均 GO，无 P0–P3 finding。
- [x] 2026-07-20：以 `3354311` 将最终 main 非重写同步到 coordinator；完整 checkpoint diff
  check 与 `make check BINARY=/private/tmp/dot-m1-cp4-coordinator-final-reopen/dot` 通过，更新
  Outcomes/Handoff，迁移本计划并创建纯计划 closure commit。
...
收口后发现的 target pair priority 缺陷形成新的单节点串行 Wave。它从 clean
`main@09147de` 创建，独立计划和 review，不重写既有 CP4 历史；只有该节点 closure 并 FF-only
合入 main 后，才重新验收整个 Checkpoint。
...
| 完整解析 target-set 内 equal 优先于 ancestor，state 损坏分类不依赖 key 顺序 | paths/runtime regressions | 通过 |
| README 准确区分内部执行内核与尚未公开的真实 apply | CLI 拒绝回归 + 独立 review | 通过 |
| 完整 Checkpoint 本地门禁 | checkpoint diff check + make check | Darwin/arm64 通过 |
...
- Observation: “pair 先于 unary”仍不足以定义 state 分类；若 pair loop 返回输入顺序中的第一个
  ancestor，较晚 equal pair 会被遮蔽。
  Evidence: `internal/paths/target_validation.go` 首个 pair early return；`internal/runtime/loading.go`
  只按返回的单个 relation 区分 `ErrCorrupt` 与 `ErrPathValidation`；四 key 组合可确定性触发。
  Impact: 完整 resolve 后的 pair 选择必须显式采用 equal 优先、无 equal 时保留首个 ancestor，
  不能让 runtime 二次解析或复制 identity 规则。

- Observation: 产品实现完成后，README 仍称真实 apply“在 executor 交付前”拒绝，把内部内核
  交付与公开 CLI wiring 混为同一阶段。
  Evidence: 首次重做 Acceptance 的规范 reviewer 对 README、`newApplyCommand` 与 runtime 前
  拒绝回归的交叉检查。
  Impact: 当前 Checkpoint 内校正 README 事实；不连接 CLI、不新增测试文案耦合，也不改变规范。

- Observation: README fix 合入后，三名独立 reviewer 重新审查完整 42-file checkpoint diff，
  执行内核、恢复链、公开边界和 Go/双平台均无 P0–P3 finding。
  Evidence: `e9e8bac...main@aa3a3f6` 三路最终 Acceptance；重复恢复/拓扑 tests、全仓 race、
  Linux/amd64 build/test-binary 编译及本地 `make check` 均通过。
...
- Decision: 完整 target-set 成功解析后，equal pair 优先于任意 ancestor pair；若无 equal，仍返回
  输入顺序中的首个 ancestor，最后才检查 unary self traversal。resolution failure 保持最先返回。
  Rationale: equal 表示 state 语义损坏，ancestor 表示语义合法 state 的运行时拓扑不安全；共享
  validator 必须提供稳定优先级，runtime 特判或错误聚合都会复制真相源或改变错误 API。
  Date: 2026-07-20

- Decision: 在 CP4 收尾修正 README 的当前实现边界，不把该 P2 延后到未来公开 apply 的节点。
  Rationale: README 是“当前实现”的事实来源；准确说明内部内核已交付且真实 apply 尚未接线，
  比保留过期阶段条件更清晰，同时不产生公开 API、行为或范围变化。
  Date: 2026-07-20

## Outcomes and Handoff

Checkpoint 4 已完成。原始三个 Milestone 按严格 DAG 独立计划、测试先行、语义提交、完整复审、
closure 后 FF-only 合入：`feat/apply-link@f4522b0`、`feat/apply-scaffold@061b783`、
`feat/apply-runtime@0a6233d`。首轮 Acceptance 的 fixture 隔离与 Store-stage 故障证据由
`fix/m1-apply-core-acceptance@eb5753d` 修复。

连续审查没有停在局部补丁：`m1-cp4-evidence-contract@9ba6964` 统一最小充分 Precondition、
executor capability 与 result/effect 协议；`m1-cp4-state-topology@34f8609` 建立 immutable
persisted baseline、candidate success-prefix 与 publish-window 恢复证明；
`m1-cp4-target-self-traversal@f3e772e` 把 unary topology 提升到共享完整 target-set；
`m1-cp4-target-pair-priority@8a0f8a1` 进一步固定
`resolution failure > equal pair > 首个 ancestor pair > unary`，使多个 state key 指向同一 target
始终分类为 `ErrCorrupt` / `ErrTargetIdentityConflict`。每组有效 finding 都有独立 test/fix commits、
完整 branch review 与门禁，无 unresolved finding。
...
Store/崩溃窗口保留可 strict-load 的旧 state，重跑由 L2/S1b 收养并稳定达到零
executor/adopt/target mutation/Store；完整 effective profile 的 pair/unary topology 不能被 partial
scope 或 state-only action 绕过。README 现准确说明内部 link/scaffold 内核已交付、尚未接入公开
CLI；非 dry-run `dot apply` 仍在 runtime 输入加载前硬拒绝。state v1、ownership、公开输出/退出码
和 M2/M3 范围均未改变，也未新增依赖。

README P2 由 `m1-cp4-readme-apply-boundary@aa3a3f6` 修复并经两份独立复审 GO；随后三名未参与
实现的 reviewer 对 `e9e8bac...main@aa3a3f6` 重新完整验收，规范/公开数据流、数据保护/恢复、
Go/测试/双平台三路全部 GO，无 P0–P3。coordinator 以 `3354311` 非重写同步该 main 后，最终
checkpoint diff check 与 `make check BINARY=/private/tmp/dot-m1-cp4-coordinator-final-reopen/dot`
在 Darwin/arm64 通过；Linux/amd64 完成生产 build 与关键 package test-binary 交叉编译，但未在
Linux 实机运行。main、coordinator 与已移除前的 fix worktree 均 clean；所有本 Goal worktree 均
无 force 移除。精确最终 HEAD 未 push，远端 macOS/Linux CI 未实际运行，因此 handoff 结论是：
**本地验收通过、远端待验收**。
