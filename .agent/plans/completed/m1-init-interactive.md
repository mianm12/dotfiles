...
- [x] 2026-07-22：Round 2 独立复核给出唯一 P2：init config-only 缺少可信 context，nested apply 又
  在 config 行之后由 projection 打印 context，违反动作输出前置顺序。config commit 后现从 locked
  inputs/candidate/env 输出一次 context；写失败保留 config、停止 config/action/child apply；nested
  projection 抑制重复 context，普通 apply 保持。修复 commit `f6ec5cf` 的 targeted race、lint、branch
  diff、Darwin/Linux compile-only 与 isolated exact-HEAD `make check` 全部通过；等待 Round 3。
- [x] 2026-07-22：未参与实现者在 `main@0844a846137b85fdba5925e5c5e72d777a548184`
  基线上完成 Round 3 全分支只读复核；结论 GO、无 P0–P3 finding。main 未变化，无 freshness 同步
  需要；当前任务明确授权仅记录最终证据并执行 active → completed 纯 plan lifecycle closure。
...
| Round 2 init context 顺序、唯一性与写失败边界 | CLI output/integration tests | 已通过 |
| Round 3 完整独立复核 | base...HEAD read-only review | 已通过，GO，无 P0–P3 |
...
- Observation: nested apply 原本只能在 runner 返回后由完整 projection 打印 context，因此 config 行
  已经先出现，且 hook 实时 stdout 也可能更早；config-only 则完全没有 planner projection 可用。
  Evidence: Round 2 reviewer finding 与 config-only/nested output order tests。
  Impact: init 必须在 child runner 前独立发布 locked context，并让 nested projection 只省略 context 行。
...
- Decision: init context 的 repo 取 locked `LoadedInit.Inputs` effective control path，profile 取已经
  locked commit 校验的 candidate，OS 取 CLI environment；context 成功写出后才打印 config 并进入 child。
  nested apply 复用原 projection body 但关闭 context，普通 apply 继续完整打印。
  Rationale: 形成单一可信前置 context，不复制 planner projection，也不 buffer 或改变 hook stdio。
  Date: 2026-07-22
...
`InitSession`，测试 seam
先真实关闭再注入错误并重新取锁证明无泄漏。Round 2 的唯一 P2 已在 `f6ec5cf` 处理并通过
exact-HEAD full gate。Round 3 在 main 仍精确为
`0844a846137b85fdba5925e5c5e72d777a548184` 时完成全分支复核，结论
GO、无 P0–P3，因此无需 freshness merge。状态：completed。本次 closure 只更新并迁移 ExecPlan，
没有修改实现、README 或测试；远端 CI、真实 Linux 和真实私人配置仍未运行，不能作为完成证据。
