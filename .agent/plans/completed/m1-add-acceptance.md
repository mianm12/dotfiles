...
- [x] 2026-07-22：Round 3 fix 后重新通过三包普通、add/CLI 5 次重复、三包 race、定向 lint、
  fix/checkpoint diff check、隔离 `make check` 与 Darwin/Linux amd64 add/CLI test binary 交叉编译。
- [x] 2026-07-22：未参与实现的 Round 4 reviewer 对完整有效基线 branch 复审 GO，无 P0–P3
  finding。主线程确认 main 仍为 `0ddb42a`、worktree clean，并重跑最终 manifest/add/CLI tests、
  fix/checkpoint diff check 与隔离 `make check` 通过。
- [x] 2026-07-22：完成 Outcomes/Handoff 与最终证据，迁移本计划到 `completed/`；不再修改实现
  或测试。
...
manifest check；每轮有效 fix 后均重新运行相应窄门禁与完整本地门禁。Round 4 reviewer 对完整
`0ddb42a...HEAD` 给出 GO，主线程随后再次完成最终 manifest/add/CLI tests、fix/checkpoint diff
check 与隔离 `make check`。双目标只完成交叉编译，真实 Linux 主机和远端 macOS/Linux CI 未运行。
...
Acceptance fix 已完成并通过四轮完整独立复核。`7621996` 从既有 prospective candidate 事实稳定
列出 module/source，并由 manifest 直接声明生成不丢成员/`@profile` 的确切 TOML 行；`ea7aa0d`
补齐 dotted profile 合法 key、expanded membership 与 module-local OS/target 激活事实；`221a46e`
把 profile、OS、target 三个可同时失败的维度组合为完整恢复步骤；`fef58ed` 在 strict Resolve 边界
分类 active module 缺 current GOOS target mapping，只对精确匹配的显式 module 投影 conflict，其他
module、无 `-m` 和其他 Resolve 错误继续原样 fail closed。

Round 1 的两个 P2、Round 2 与 Round 3 各一个 P2 均由新的 fix commit 测试先行关闭；用户明确授权
突破三轮上限后，Round 4 reviewer 对完整 branch 给出 GO，无 P0–P3 finding。最终本地通过
manifest/add/CLI 普通测试、add/CLI 5 次重复、三包 race、定向 lint、fix/checkpoint 完整 diff check、
隔离 `make check` 和 Darwin/Linux amd64 add/CLI test binary 交叉编译。真实 Linux 主机及远端
macOS/Linux CI 未运行，远端待验收。

本计划现迁移至 `completed/`；handoff 为主 agent 确认纯计划 closure commit、branch/worktree
clean，并按 acceptance fix freshness/fast-forward 流程集成本地 main。无 unresolved blocking
finding、依赖、持久化格式、ownership 或 mutation 边界变化。
