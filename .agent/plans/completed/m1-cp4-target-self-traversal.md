...
- [x] 2026-07-20：先提交 paths、partial apply 与 S1b state-only 失败回归；旧实现分别返回
  validator/Run 成功，真实证明 scope 和 execution class 均可绕过。
- [x] 2026-07-20：在共享 target-set validator 实现 unary self-traversal gate；paths/manifest/
  planner/apply/runtime/state 窄测、race、关键回归 `-count=10`、Linux/amd64 交叉编译、diff check
  与完整 `make check` 通过。
- [x] 2026-07-20：独立完整复核 round 1 中两路 GO；数据保护 reviewer 发现有效 P1：过早返回
  unary error 会遮蔽多 state key equal identity 的 corrupt 分类。test-first 证明 combined fixture
  被误分为 `state.ErrPathValidation`。
- [x] 2026-07-20：保留原有 resolve→pairwise precedence，仅在无 pair conflict 后报告 unary；
  paths/runtime 组合回归、六包窄测/race、关键回归 `-count=10`、Linux 交叉编译、diff check 与
  `make check` 再次通过。
- [x] 2026-07-20：独立完整复核 round 2 中规范与数据保护两路 GO；Go/架构 reviewer 发现有效
  P2：Milestone 2 和早期 Decision 仍描述已被 supersede 的 unary-first 顺序。
- [x] 2026-07-20：校正 living ExecPlan，使 Milestone、Decision 与当前 resolve→pair→unary error
  contract 一致；生产代码无需修改。
- [x] 2026-07-20：最终 round 3 三位未参与实现的 reviewer 全部 GO，无 P0–P3 或 remaining
  blocking finding；current main 仍精确等于 effective base，final branch gate 通过，完成 plan
  closure。
- [x] 2026-07-20：将 clean、可 fast-forward 的 acceptance-fix branch 交还 coordinator；本计划
  收口后由主 agent FF-only 集成 main，并重新验收完整 `checkpoint_base...main`、更新 coordinator
  最终交接。
...
先在同一 resolver snapshot 内解析全部 labeled targets，并保留既有 pairwise conflict 的优先级；
只有集合不存在 equal/ancestor pair 时，才逐 target 验证 `resolution.Traverses(resolution)`。unary
失败返回零值集合、保留 label/path provenance 并 wrap `ErrTargetOverlap`。使用独立 unary error
shape，不把同一 target 伪装成 `TargetConflictError` 的左右双方，也不扩张 `TargetRelation` 的 pair
语义；上层 manifest/control boundary 入口保持不变。
...
- Observation: 共享 target-set gate 同时用于 strict state path validation；旧的
  `~/detour/bridge` historical fixture 本身也是 self traversal，因此修复后在 locked load 阶段就
  被拒绝，不再到达 planner 的 persisted-baseline gate。
  Evidence: 首次完整窄测让原回归由预期 planner error 变为
  `state.ErrPathValidation` + `paths.ErrTargetOverlap`；state bytes/target/Store 仍为零 mutation。
  Impact: 将该回归改为明确验证 strict state consumer 的更早拒绝；另一个 L1 + 合法 descendant
  historical fixture 与 planner 纯函数矩阵继续独立证明 persisted-baseline 恢复门禁，证据未丢失。

- Observation: 同一 target-set 可以同时包含 unary self traversal 和两个 key 的 equal identity；
  runtime 依赖结构化 `TargetConflictError(Equal)` 把后者分类为永久 state corruption。
  Evidence: round 1 reviewer 的 `~/bridge` + `~/detour/bridge` fixture 在首版 gate 下只返回
  `TargetSelfTraversalError`，丢失 `state.ErrCorrupt`/`ErrTargetIdentityConflict`。
  Impact: validation 顺序也是 error contract；解析和 pair conflict 保留既有优先级，unary 仅作为
  集合无 pair conflict 时的完整性补充。

## Decision Log

- Decision: 最初计划在 `ValidateTargetSet` 解析每个 target 后立即做 self-traversal gate；round 1
  证明该顺序会遮蔽更具体的 pair error，本决定已被后续 resolve→pair→unary 决定 supersede。
  Rationale: 这是完整 structural desired 的 unary invariant；这里最早拥有同一 resolver snapshot、
  provenance 和所有调用方，位置选择仍成立，但即时返回的顺序不成立。
  Date: 2026-07-20
...
- Decision: 全部 target 解析后先检查既有 pair relations，再检查 unary self traversal。
  Rationale: pair error 携带双方 provenance 和 equal/ancestor 的更具体事实；保持其优先级既不改变
  历史 API，又让 state consumer 能继续区分永久 equal-identity corruption 与当前 topology unsafe。
  Date: 2026-07-20

## Outcomes and Handoff

已完成。实质与 review-fix commits：

- `0b7997e`：用 paths 单 target、partial `Run` 和 S1b state-only `Run` 固定旧实现的三条放行路径。
- `abf081c`：在共享 `ValidateTargetSet` 增加结构化 unary gate，并让 strict state consumer 明确在
  load 阶段拒绝同一非法拓扑。
- `e9a6bf2`：固定 self traversal 与 equal identity 并存时仍须返回 pair conflict/corrupt 的组合回归。
- `6fbef01`：恢复 resolve/pairwise 优先级，仅在没有 pair conflict 时报告 unary self traversal。
- `eb3ffe9`：校正 living ExecPlan 中已被 supersede 的 unary-first 顺序描述。

本地 paths/manifest/planner/apply/runtime/state 窄测与 race、关键回归 `-count=10`、
Linux/amd64 apply test binary 交叉编译、`git diff 34f8609...eb3ffe9 --check` 和精确 review HEAD 的
`make check BINARY=/private/tmp/dot-m1-cp4-self-traversal-final-review/dot` 全部通过。round 1 的
error-priority P1 与 round 2 的 living-plan P2 均以新 commit 修复；round 3 三路完整复审全部 GO，
没有 unresolved finding。

worktree 在收口时 clean，branch 与 current main 的 merge-base 仍为 effective base `34f8609`，可由
主 agent fast-forward-only 合入。整个 `checkpoint_base...main` Acceptance 和 coordinator closure
尚未在本计划内声称完成。远端 macOS/Linux CI 未运行；Linux 证据仅为交叉编译。
