...
- [x] 2026-07-22：Milestone 3 完成 control-bound `InitCandidate`、`LoadedInit` 单次成功 config
  commit capability、锁内 strict preflight/manifest refresh、失败可重试和 nested mutation gate；
  `go test -race ./internal/runtime ./internal/config` 与全仓非 race tests 通过。
- [x] 2026-07-22：freshness gate
  ...
  完整通过（tidy diff、format、lint、全仓 race、build、synthetic manifest）；goal-base diff check
  通过。已更新 review-ready handoff，按父任务边界保留计划在 `active/`，等待独立复核/集成协调，
  不执行 completed 迁移或 closure commit。
- [x] 2026-07-22：Round 1 独立复核确认三项有效 finding：missing Precond 的最终 rename
  可覆盖竞争创建、interaction selection 错误高于显式 profile override、publisher fault seam/矩阵不足。
  追加 `184e20c`/`ed18759` 修复：missing 发布使用同目录已 sync temp 的 atomic hard-link
  no-replace，`EEXIST` 映射 `ErrPreconditionChanged`；profile override 在 pure candidate 与 locked
  validation 两处强制；fault matrix 覆盖 create/chmod/write/short-write/sync/close/final publish/
  cleanup remove。`go test -race ./internal/config ./internal/runtime` 通过。
- [x] 2026-07-22：Round 1 修复后的 branch diff check 与独立 cache `make check` 再次完整通过
  （tidy diff、format、lint 0、全仓 race、build、synthetic manifest）；计划继续保持 active，等待
  后续独立复核/父任务集成，不迁移 completed 或创建 closure commit。
- [x] 2026-07-22：Round 2 复核确认 post-link cleanup P1：missing publish 已建立完整 final 后，
  remove(temp) 失败会返回 changed+error，但 runtime 错误地保持 config gate 关闭且同 capability 无法
  重试。测试先行引入密封 `config.PublishResult`，显式区分 Changed/Committed/error；no-op、rename、
  link 越过发布点均 committed，pre-publish failure 不 committed。`LoadedInit.CommitConfig` 先同步
  committed gate 再原样返回 cleanup error，外部不会吞错或误认为可重试未提交；真实 remove fault
  覆盖 final 0600/bytes、temp 遗留、gate 打开、二次 commit 拒绝。config/runtime race tests 通过，
  修复提交为 `a7bd19d`。
- [x] 2026-07-22：Round 2 修复后的 base...HEAD diff check 与独立 cache `make check` 完整通过
  （tidy diff、format、lint 0、全仓 race、build、synthetic manifest）。worktree 交接前 clean；计划
  继续保持 active，等待强制完整 Round 3，不迁移 completed 或创建 closure commit。
- [x] 2026-07-22：freshness merge `42473471f5cc0c403c3d069a2cc85cb11a169459`
  同步已完成的 CP7 hook 基线后，未参与实现者对完整当前分支执行 Round 3 只读复核；结论为无
  P0–P3 finding、GO。复核确认 Round 1/2 修复、Scope / Non-goals、数据保护边界和验证证据均闭合。
- [x] 2026-07-22：当前任务明确授权仅更新本计划并执行 `active/` → `completed/` lifecycle closure；
  本次纯计划 closure commit 不修改实现、README 或测试，完成后 worktree clean。
...
| 必须成立的性质 | 验证证据 | 状态 |
|---|---|---|
| manifest/config snapshot 完整且不可变 | manifest/config 单元测试 | 已通过 |
| prepare lock-free、state-free、零写入 | runtime operation events + synthetic filesystem | 已通过 |
| merge/selection 区分 omitted 与 explicit empty | config/runtime table tests | 已通过 |
| repo provenance 与 override 持久化正确 | paths/runtime/config tests | 已通过 |
| candidate 整体严格有效且 encoding 稳定 | config candidate tests | 已通过 |
| 0600 atomic publish、cleanup、Precond/no-op 正确 | config publisher/fault tests | 已通过 |
| config commit capability 与 nested mutation gate 正确 | runtime lifecycle/recovery tests | 已通过 |
| Go、依赖、lint、race、build、manifest 门禁 | isolated-cache `make check` | 已通过 |
| freshness merge 后完整独立复核 | Round 3 full-branch read-only review | 已通过，无 P0–P3 finding，GO |
...
最终验证证据包括各 milestone 的 targeted/race tests、Round 1/2 修复后的 base diff check，以及
独立 `/private/tmp` cache/BINARY 的完整 `make check`（tidy diff、format、lint 0、全仓 race、build、
synthetic manifest）。`4247347` freshness merge 后的完整只读 Round 3 再次给出 GO；远端 CI、
真实 Linux、真实私人配置均未运行。
...
上述限制记录的是实现阶段授权；2026-07-22 当前 closure 任务另行明确授权仅更新本计划、迁移
`active/m1-init-config.md` 到 `completed/m1-init-config.md` 并创建纯计划 closure commit，不授权任何
实现、README、测试、托管或历史改写操作。
...
- `BeginInit` 的首次 preflight 发生在 lock 前，因此锁内 refresh 若解析出不同 control-plane paths，
  既有 ownership 并不保护新路径。`InitSession.Load` 对这种变化直接按 Precond 失配拒绝；profile/data
  等不改变 control paths 的合法变化可 strict load，但旧 preparation candidate 随后仍会因 bytes/mode
  Precond 失配而拒绝发布。
- 首轮 freshness gate 在测试前的 lint 阶段报告 error wrapping、repo source 无效初值和导出常量注释
  三个局部问题；最小修正后相关 config/paths/runtime race tests 通过，需在 fix commit 后重跑完整门禁。
- Round 1 复核指出 missing Precond 即使在 final precheck 仍缺失，随后 `os.Rename` 仍会无条件覆盖
  窗口内出现的配置；现有标准库 `os.Link(prepared, destination)` 在同目录提供 atomic no-replace，
  并在当前 Darwin 测试环境验证 `EEXIST` 竞争路径、完整 bytes、0600 和 cleanup，无需依赖或平台特例。
- Publisher fault matrix 需要区分“可清理路径”和“cleanup 本身失败”：前者不留 temp；后者保留可诊断
  temp 且用 `errors.Join` 同时暴露 primary/remove 错误，不能把 cleanup failure 报成成功。
- Round 2 证明 `error != nil` 不能代表“未提交”：hard-link 已成功时 final 已是完整 config，之后只剩
  temp 名称 cleanup。若 runtime 丢弃这个 committed 事实，capability 状态会与磁盘不可逆事实分裂。
...
- 2026-07-22：config commit 失败不消耗 capability，允许同一持锁 session 在可恢复 publisher 故障后
  重试；第一次成功（含 equivalent no-op）才关闭额度并打开 nested mutation gate。
- 2026-07-22：missing Precond 的最终 publish 不再复用 existing replacement 的 rename。missing 分支
  以 hard link 建立 final；已存在 destination 绝不覆盖并映射 Precond 变化，成功后移除 temp 名称。
  existing 分支继续使用 final strict recheck + rename，保留规范已接受的 replacement window。
- 2026-07-22：`InitContext.ProfileOverride` 是不可被 interaction selection 改写的 CLI 事实；因此
  BuildCandidate 先消费 override，locked commit 再复核 candidate profile 精确相等，防止来自另一份
  preparation 的合法但错误 profile candidate 越过 capability gate。
- 2026-07-22：publisher/runtime 之间以单个密封 `PublishResult` 传递 changed/committed，error 只承载
  失败信息，不再参与提交状态推断。post-commit cleanup error 保留 `Committed=true` 并关闭 capability；
  pre-commit error 返回 zero result 且仍可在同 session 重试；成功 no-op 同样 committed 并打开 gate。
- 2026-07-22：freshness merge 后 Round 3 完整独立复核无 finding 并 GO；因此按当前明确 closure
  授权把同一计划迁入 `completed/`。该 lifecycle 变化只表达本 Goal 已闭合，不扩展 init CLI 等后续范围。

## Outcomes and Handoff

实现已在 `feat/init-config` 形成以下 checkpoints：

    3fa6e3f plan(init): 冻结机器配置内核范围
    c767a6e feat(init): 建立只读配置准备快照
    1dd5603 feat(config): 原子发布严格 init 候选
    c195c7b feat(runtime): 绑定 init 配置提交 capability
    e1e35ce fix(init): 满足配置边界静态检查
    184e20c fix(config): 关闭缺失配置发布竞态
    ed18759 fix(init): 强制显式 profile override
    a7bd19d fix(init): 同步发布后清理错误的提交事实

状态：completed。freshness merge `4247347` 后的 Round 3 完整独立复核无 P0–P3 finding 并 GO。

最终结果覆盖 Scope 中的 immutable data declarations、完整 machine/Precond snapshot、repo source、
lock-free preparation、pure candidate selection、0600 atomic publication、locked strict refresh、一次
成功 config commit capability 和 nested mutation gate；未接入 Cobra/TTY/`--set`/apply，也未修改
state、CLI I/O 或规范。Round 1 后 missing 配置还具备 atomic no-replace，显式 profile override
无法被 interaction 或跨 preparation candidate 绕过，publisher failure/cleanup 证据完整。窄范围 race
与各轮全仓 freshness gate 均通过。Round 2 后 publisher/runtime 还显式保留 post-commit cleanup
error 的 changed/committed 事实，capability gate 不再与磁盘分裂。远端 macOS/Linux CI、真实 Linux
和真实私人配置未运行。本次只执行计划 lifecycle closure；没有修改实现、README 或测试，也没有
执行 merge/push/rebase/amend/reset 等操作。后续 Cobra init、TTY/`--set` 与 apply 接线仍属于新的
独立 milestone，不是本 Goal 遗留工作。
