...
- `docs/cutover-plan.md` 是非规范性的重构切换记录，只用于检查点追溯；与基线冲突时以基线
  为准，新增维护不以它作为产品路线图。
- `README.md`、代码和测试说明当前已经实现什么，不能用目标设计代替实现证据。
- `docs/archive/` 仅用于历史追溯，没有规范性，也不得指导新增实现。
- `CONTRIBUTING.md` 规定 Git 和通用开发约定。

修改 `docs/design-baseline.md` 或 `docs/cutover-plan.md` 前必须获得用户明确授权；自主检查点
执行期间两者始终只读。
...
继续、复核或重跑 `docs/cutover-plan.md` 的检查点时，必须使用仓库 skill
`$dot-checkpoint`（`.agents/skills/dot-checkpoint/SKILL.md`）。

- 一次运行只执行一个检查点；无法满足退出条件时停止并报告偏差，不得即兴绕过。
- 检查点执行期间 `docs/design-baseline.md` 与 `docs/cutover-plan.md` 只读。需要修改时按基线
  §14 报告失败案例、成本与不实现后果，交人裁决。
- Git 授权止于工作分支：可按 skill 小步 commit；不 merge、不推送 main、不打 tag。
- 只有 B2、B3 的互不重叠实现与检查点晋级评审可以并行；任何 rebase 或历史重写仍需用户明确授权。
- Codex 评审角色定义在 `.codex/agents/`；完整评审契约以 `$dot-checkpoint` references 为准。
- 跨完整计划的 goal 每次自动续跑仍最多晋级一个检查点；C2 真实机器切换必须在到达时另行取得
  用户明确授权。
