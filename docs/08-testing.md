...
| `planner/decide` | 05 号文档 §3.2/§3.3 决策表**逐行**:L1–L6(含 L3 存证重链、L4 改指 conflict)、M1–M6(含 M3a/b/c 三分支与 --adopt 开关)、S1–S3、P1–P3;**conflict 存在 → 全部 Prune 带 Deferred 标记** |
| `planner/validate` | target 重复、`foo` vs `foo.tmpl` 碰撞、祖先冲突(`Within` 非字符串前缀:`/home/a` vs `/home/ab`)、中间目录穿文件 |
| `executor` | **过期 Precond 注入**:喂 scan 后被篡改的 Action(Missing→已存在、hash 已变、LinkDest 已变)→ 动作降级 Conflict、文件未动、prune 转 deferred;--force 备份失败 → 放弃该 BackupReplace |
...
13. **add 全分支**:唯一命中自动归入;多候选退出码 3;`-m` 新建模块 + ∉ profile 报错 +
    `--activate` 改顶层 manifest;`*.local` 硬拒绝;**--template:原文件保留、
