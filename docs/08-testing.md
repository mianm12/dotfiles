...
`HOME`/`XDG_*` 环境**全部重定向——最后一项靠 executor 统一从 `paths` 取子进程注入
...
| 包 | 重点用例 |
|---|---|
| `manifest` | 两阶段加载(宽松预读只取 requires;严格解码未知键即错;doctor 宽松);合并整键覆盖;ignore 并集特例与优先级(含 hooks 引用不可被 [files] 覆盖 → 校验错误);profile @ 展开/环/悬空;[data] 小写命名;target 双形态互斥;**[target] 表缺当前 GOOS(os 过滤通过时)→ 硬错误**;路径合法性(模块名 `../x` 拒绝、[files] 键含 `..`/绝对路径拒绝) |
...
| `planner/validate` | target 重复、`foo` vs `foo.tmpl` 碰撞、祖先冲突(`Within` 非字符串前缀:`/home/a` vs `/home/ab`)、中间目录穿文件;**StateOp×Kind 合法组合断言**(Delete⇔Prune、Upsert⇒NextEntry≠nil、Skip/Conflict/RunHook⇒Keep,违规即报错) |
| `executor` | **过期 Precond 注入**:喂 scan 后被篡改的 Action(Missing→已存在、hash 已变、LinkDest 已变)→ 动作降级 Conflict、文件未动、prune 转 deferred;**BackupReplace 分型**:普通文件 copy+hash、symlink 备份链接本身(不跟随)、目录/特殊文件即使 --force 仍拒绝;备份目录 O_EXCL 排他创建、**文件保留原 mode**(可执行位不丢);备份失败 → 放弃该 BackupReplace;StateOp/NextEntry 仅在动作成功后执行(Delete 仅随 Prune,Keep 绝不触碰记账);**防御式复检非法组合 → 报错终止** |
...
| `state` | **三态 + 语义校验**:缺失 → 正常空载;损坏 JSON → fail-closed;version+1 → 同;**kind 非法 / target 出 HOME / symlink 缺 link_dest / rendered 缺 hash → 视同损坏 fail-closed**;原子写;flock 互斥;link_dest 序列化 |
| `fsutil` | 原子写中断模拟、symlink 覆盖替换、备份 copy+校验、权限(0600/0700/保留 mode) |
| hooks 指纹 | (路径,长度,内容)元组编码:内容拼接歧义用例(`"ab","c"` vs `"a","bc"`)必须产生不同指纹;**指纹不含 profile/data**(切 profile 后指纹不变、不重跑) |
...
2. **conflict 三态 + 收敛门控**:预置真实文件 → apply 退出码 3、该文件未动、其余创建
   已完成、**全部 prune 输出 deferred 且未执行** → `--force` 后备份存在(**保留原
   mode**)、内容替换、prune 恢复执行。
...
6. **死链 prune(ADR-22 回归)**:apply 后从仓库删除源文件 → 旧链接悬空 → 全量 apply
   正常将其 prune(解析式 owned 在此恒失败,本用例防止回归)。
...
9. **整模块孤儿确认**:切换 profile 使某模块整体退出 → 无 `--yes` 时要求确认,输入 N
   → **本次全部 prune 延迟、退出码 2**;`--yes` 直接清理。scaffold 产物始终不删。
...
13. **add 全分支**:唯一命中自动归入;多候选退出码 3;`-m` 指向不存在模块 → 报错 +
    两步指引(**不自动创建**);∉ profile 报错并打印待添加行(CLI 不改 manifest);
    `*.local` 硬拒绝;目录/symlink/特殊文件拒绝;**--template [M2]:原文件保留、
    kind=rendered、替换变量后 apply skip(闭环)**;add 后幂等。
14. **state fail-closed**:损坏 state.json → apply/add 拒绝(exit 1)、status/doctor
    正常诊断;`mv` 备走后 apply 恢复并经收养重建;version+1 同理;**语义校验**:手工
    注入 kind 非法 / target 出 HOME / 缺 link_dest 的条目 → 同样 fail-closed。
...
18. **run_once**:首次执行、指纹不变跳过、脚本内容(或 watch 文件 [M2])变化重跑、
    ...
    **执行细节**:工作目录 == 模块目录(脚本写 `./marker` 落在模块内);无可执行位
    脚本经 sh 正常执行;**hook 环境隔离**:脚本写 `$HOME/marker` 与
    `$XDG_CONFIG_HOME/marker` → 断言落在 fake home 内、真实家目录无写入。
19. **锁边界**:持锁下第二个 apply 立即报错;diff/status 不受锁影响;update 在洁净
    检查与 pull 阶段即持锁(可用慢速 hook 模拟占锁窗口)。
20. **kind 迁移(误删回归)**:apply 生成 rendered 产物 → 仓库把 `.tmpl` 改为
    `.template` → apply 产出 Adopt(记账 → scaffold)、文件未动 → 移除模块后全量
    apply → **文件仍在**(P1)。另测 owned link → managed 的无感模板化
    (`git mv foo foo.tmpl` 后 apply:链换产物、记账迁移、无 conflict);
    以及 **entry=nil 回归**:把 owned 链接手工换成内容恰好等于渲染结果的普通文件 +
    转 managed → 仅提示 adoptable、绝不自动收养,此后 prune 永不删该文件。
21. **add 失败无副作用**:注入 symlink/rename 失败(只读目录等)→ 原路径文件仍在且
    内容未变、**本次仓库副本与临时链已被自动清理**,直接重试成功。
...
24. **add 仓库侧安全**:仓库已有 `foo.tmpl` 时 `add foo` 拒绝(后缀碰撞);state
    ...
    rename 前修改原文件 → add 中止、原文件保留新内容、仓库副本被自动清理;多路径 add
    中任一预检失败 → 全部不执行。
25. **add 解析链预检(ADR-30)**:`add ~/README.md`(命中全局 ignore)、add 落点在
    模块 `hooks/` 下、add 命中模块 ignore pattern → 均在任何写入前被拒绝并指出规则,
    仓库与 `$HOME` 零改动;对照:正常文件通过预检后,下轮全量 apply 的计划为空
    (add 产物即合法 desired,不产生孤儿)。
26. **update 洁净检查**:仓库有未跟踪文件或本地修改 → update 在 pull 前报错、仓库
    未动;清理后 update 正常走完 pull → requires → apply。
...
- 追加:`go vet`、`golangci-lint`、`dot doctor --manifest-only`(用刚构建的二进制 lint
  仓库自身配置,M1 即提供该最小子集;**已跟踪的 `*.local` 在此模式判为错误**)。
