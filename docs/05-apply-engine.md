...
`[files].kind`),按三原则迁移,**一切记账变更经 `NextEntry` 在动作成功后提交**:

> 所有权只能被证据延续,不能被类型切换凭空创造;
> 迁入 scaffold = 释放所有权,永远安全;迁出 scaffold = 等同无记录。

| 旧 kind(state) | 新 kind(desired) | 规则 |
|---|---|---|
| rendered | scaffold | **无条件 metadata-only**:文件不动,产出 Adopt 把记账改为 scaffold(释放所有权;drift 与否无关——终点是「归用户」,与任何现内容兼容)。此后模块移除时命中 P1,永不删 |
| link | scaffold | 旧证据 owned(Readlink==link_dest)→ 自动:删链 + 生成 scaffold 产物,记账迁移(删自家链零损失,用户得到真蓝本);不 owned → 目标既存 → S1 skip,同时产出 Adopt 将旧记账释放为 scaffold |
| link | managed | owned → 自动替换:render 覆盖自家链,记账迁移(最常见迁移:`git mv foo foo.tmpl` 模板化既有配置,应当无感);不 owned → 走 M 表常规决策(通常 CONFLICT) |
| rendered | link | owned(hash==记录)→ 自动:create-link 替换自家产物,记账迁移;不 owned → CONFLICT-drift |
| scaffold | link / managed | scaffold 无所有权,**等同 state 无记录**:走新 kind 常规决策(存在 → conflict / adoptable;缺失 → 创建);成功后记账迁移 |

v1.2 的缺陷示例(本节的动机):`foo.tmpl` 曾生成文件(state=rendered)→ 仓库改成
`.template` → 旧规则 S1 skip 而记账仍是 rendered → 未来移除模块时 P2 按旧 hash 判
owned 并删除——违反 ADR-12。迁移表第一行在切换发生的那次 apply 就把记账改为
scaffold,路径闭死。原子的「切换 + 移除」同一次发生时(中间从未 apply),记账仍是
rendered 且 hash 匹配 → 按 owned rendered 产物清理,属契约之内。
...
- 备份(BackupReplace 前置,ADR-29):**普通文件** copy + hash 校验;**symlink** 在
  ...
  文件 0600;备份失败则放弃该 BackupReplace。
...
7. 执行(按模式分支):
   默认(link):   copy → modules/<m>/<rel>(保留 mode,含可执行位);校验副本 hash;
                   目标同目录创建临时 symlink(.dot-tmp-*)→ 原子 rename 覆盖原文件
                   ——rename 前原文件未动,换链失败时必然完好;
                   登记 kind=symlink + link_dest。
