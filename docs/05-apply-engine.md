...
**加载校验与三态处理(ADR-25)**:JSON 解析之外,加载时做**语义校验**——每条 entry
的 kind 必须为合法枚举、target 键必须位于 HOME 之内(`paths.Within`)、各 kind 的
证据字段必须完整(symlink 必有 `link_dest`,rendered 必有 `hash`);任一违规视同损坏。

| state 状况 | 行为 |
|---|---|
| 文件缺失 | 合法的全新开始:无历史可 prune、无 drift 检测,正常运行 |
| 解析失败 / **语义校验失败**(损坏) | **fail closed**:mutation 命令拒绝执行;`status`/`doctor` 可运行并报告;M1 恢复路径 = 手动 `mv state.json state.json.bak`(把「损坏」显式转化为「缺失」),重跑 apply 后由收养规则重建 |
...
| L2 | T 是 symlink ∧ Readlink == 期望 dest | skip;state 无记录**或记录元数据过期**(link_dest/source/module 与现值不符——如 L3 重链成功但落账前崩溃)→ **adopt**(自动补录/刷新,ADR-21:证据无歧义,删链最坏零损失) |
...
| L6 | T 是普通文件 / 目录 / 特殊文件 | CONFLICT(普通文件:`--force` → BackupReplace;目录/特殊文件:`--force` 也拒绝,提示手工移走,ADR-29;建议 `dot add` 收编) |
...
| M3a | actualHash == newHash ∧ Mode == mode ∧ state 有记录 | skip;记录元数据过期 → 改产出 Adopt(reason="metadata")完成刷新 |
| M3b | actualHash == newHash ∧ Mode ≠ mode ∧ state 有记录 | render(reason="mode";重写同样字节顺带落权限,ADR-26) |
| M3c | actualHash == newHash ∧ state 无记录 | 默认 skip + 提示 adoptable;`--adopt` [M2] → Adopt(Mode 不符则改产出 render,一步完成记账与修权限) |
...
模板删除/模块退出 profile 后变成可自动 prune 的产物。
...
**元数据刷新通则**:凡决策为 skip 而 entry 的非证据元数据(module、source、link_dest)
与本次 desired 不符 → 改产出 Adopt(reason="metadata")。M3a 与 L2 是其特例;
scaffold 同 kind 但跨模块移动或源改名(module/source 变化)同样适用——S1 skip 时
顺带刷新记账,否则陈旧元数据永无自愈机会。
...
`[files].kind`),按三原则迁移,一切记账变更经 StateOp/NextEntry 在动作成功后提交:

> 所有权只能被证据延续,不能被类型切换凭空创造;
> 迁入 scaffold = 释放所有权,永远安全;迁出 scaffold = 等同无记录。

**统一分派规则**(实现即这三行,下方矩阵只是其展开):

1. **迁入 scaffold**:无论旧证据成立与否,记账一律释放为 scaffold(释放所有权不需要
   证据)。旧证据 owned 的 symlink 额外享受自动转换(删自家链 + 生成蓝本);其余情形
   按 S 表决策,skip 时顺带产出 Adopt(kind→scaffold)。
2. **迁向 link/managed,旧证据 owned**(symlink:Readlink==link_dest;rendered:
   hash==记录)→ 自动替换产物,记账迁移。
3. **迁向 link/managed,旧证据不成立或旧 kind 为 scaffold** → **以 `entry=nil` 进入
   新 kind 决策表**:旧记录不得提供任何所有权证据——否则「用户换上的、内容恰好等于
   渲染结果的普通文件」会被 M3a 借旧账自动收养、日后被 prune 误删,重蹈 ADR-21 覆辙。
   由此:同内容普通文件只落 M3c(adoptable,需显式 `--adopt`);目标恰好已是精确期望
   链接 → L2 自动收养;目标缺失 → L1/S3 正常创建;其余 → conflict。旧记录保留至
   成功动作以 Upsert 覆盖,期间每次运行同样适用本规则,不会自我毒化。

| 旧 kind(entry) | 新 kind(desired) | 展开 |
|---|---|---|
| rendered | scaffold | 规则 1:metadata-only,文件不动,Adopt(kind→scaffold);此后模块移除命中 P1,永不删 |
| symlink | scaffold | 规则 1:owned → 自动删链 + 生成蓝本;不 owned → 按 S 表(存在 → S1 skip + Adopt 释放;缺失 → S3 生成) |
| symlink | managed | 规则 2/3:owned → render 覆盖自家链,记账迁移(`git mv foo foo.tmpl` 模板化既有配置,应当无感);不 owned → entry=nil 走 M 表 |
| rendered | link | 规则 2/3:owned → create-link 替换自家产物;不 owned → entry=nil 走 L 表(恰是期望链接 → L2 收养;缺失 → L1;其余 → conflict) |
| scaffold | link / managed | 规则 3:entry=nil 走新表;成功后记账迁移 |

**词汇表对应**:desired kind 的 link/managed 对应 `Entry.Kind` 的 symlink/rendered
(02 号文档 §6 强类型约定),本表两列各按自己的词汇书写,实现经单一映射函数换算。

v1.2 的缺陷示例(本节动机):`foo.tmpl` 曾生成文件(entry=rendered)→ 仓库改成
`.template` → 旧规则 S1 skip 而记账仍是 rendered → 未来移除模块时 P2 按旧 hash 判
...
- **单实例锁(ADR-19)**:mutation 命令(含 [M2] 的 state rebuild / edit)对
  `~/.local/state/dot/lock` 取 flock,失败即报错;`update` 自洁净检查与 `git pull`
  起持锁。**进程内只获取一次**:顶层命令取锁,init/update 的内部编排调用假定已持锁的
  `applyLocked()`,引擎入口不自行取锁(flock 对同进程新 fd 会阻塞,双重获取即自锁)。
  `diff`/`status`/`doctor` 只读不取锁——state 经原子 rename 写入,只读方看到完整的
  旧版或新版。
- **崩溃恢复**:state 落盘原子;「已执行未记账」窗口由收养规则收敛——重跑 apply,
  L2 自动补录/刷新,M3c 提示后 `--adopt` 补录。父目录 fsync 级持久化不做(01 §4
  已接受)。
...
4. **StateOp × Kind 合法组合断言**(02 号文档 §6):违规即 planner bug,直接报错。

`dot doctor` 复用 1–3;路径合法性(03 号文档 §6)在更早的解码阶段执行。
...
复核是免费的:裸 `os.Symlink` 遇到已存在目标天然 EEXIST;其余 rename 覆盖路径复核后
仍有微秒窗口,为已接受剩余风险(01 §4)。flock 只防另一个 dot,防不了编辑器与应用
程序——这正是复核必须扩展到全部 mutation 的原因。
...
- 换链接:`Symlink` 到临时名 + `Rename` 覆盖(同卷 rename 原子);新建链接直接
  `os.Symlink`(见上)。
- 备份(BackupReplace 前置,ADR-29):**普通文件** copy + hash 校验;**symlink** 在
  备份目录重建同 dest 的链接(备份链接本身,不跟随、不复制指向内容);**目录与特殊
  文件不可 BackupReplace**——即使 `--force` 也维持 CONFLICT,提示手工移走。备份目录
  名 = RFC3339Nano + 短随机后缀,`O_EXCL` 排他创建,不可能覆盖既有备份;目录 0700
  (即保护边界),文件**保留原 mode**(不丢可执行位);备份失败则放弃该 BackupReplace。
...
2. **自动破坏性动作 = owned() ∧ Precond;强制替换 = 显式 `--force` ∧ 备份成功 ∧
   Precond;全部 mutation = Precond 复核**。
...
- **执行指纹** = 对 `(路径, 内容长度, 内容)` 元组序列(script 在前、watch 按声明序)
  做 sha256,长度前缀编码——消除裸内容拼接的边界歧义。**指纹不含 profile/data 等
  运行上下文**(ADR-31):否则切 profile 或改一个模板变量就全量重跑;hook 需要上下文
  时读 `DOT_*` 环境变量并自我幂等。RunHook 动作携带 Fingerprint,**成功后**写入
  state 键 `run_once["<module>/<script>"]`;失败不写,下次重试。
...
4. 决策:恰一强候选 → 采用;零强候选且恰一候选 → 采用;
   其余 → 退出码 3,列出候选,要求 -m(-m 须满足模块名合法性;指向不存在的模块 →
   报错并打印两步指引:mkdir modules/<m> + 编辑 [profiles],不自动创建)。
5. profile 校验(ADR-18/28):目标模块 ∉ profile → 报错,打印需手动加入
   [profiles] 的确切行;CLI 不代改 manifest。
6. 类型校验:只接受普通文件;目录、symlink、特殊文件拒绝(目录请逐文件 add)。
7. 预检 = 完整解析链模拟(ADR-30;多路径输入先全部预检,任一失败则全部不执行):
   - 把新 source(按模式含后缀)虚拟加入模块文件树,跑 resolve + enumerate + ignore,
     要求恰好产生一个 desired entry,且 target == 输入路径、kind 符合模式;
     命中内置忽略(dot.toml/hooks//*.swp)、[hooks] 引用或 ignore pattern → 拒绝
     并指出规则——add 的产物必须是下轮 apply 的合法 desired,否则下轮即被当孤儿删除;
   - 复用 §5 的 target 唯一性/后缀碰撞校验(输入之间、输入与既有 desired 之间);
   - 仓库目标已存在(含 .tmpl/.template 后缀变体)→ 拒绝,绝不覆盖——state 丢失时
     仓库文件是唯一真相;
   - 记录各原文件的 hash 与 mode 快照。
8. 执行(按模式分支;仓库副本一律 O_EXCL 排他创建,绝不截断既有文件;
   持久化顺序:copy → chmod → Sync → close → hash 校验):
   默认(link):   copy → modules/<m>/<rel>(保留 mode,含可执行位)→ Sync → 校验
                   副本 hash;目标同目录创建临时 symlink(.dot-tmp-*);**rename 前
                   按预检快照复核原文件 hash/mode**——失配(应用程序刚写入了新内容)
                   或任何一步失败 → 自动清理本次仓库副本与临时链、保留原文件退出,
                   整体无副作用可直接重试;复核通过 → 原子 rename 覆盖原文件;
                   登记 kind=symlink + link_dest。复核后的微秒竞态为已接受剩余风险。
   --template:    [M2] copy(同上持久化顺序)→ modules/<m>/<rel>.tmpl;原文件留在
                   原位作为产物;登记 kind=rendered(hash = 当前内容);打印「请将
                   机器相关值替换为 {{ .var }}」——替换后渲染一致则下次 apply skip。
   --scaffold:    copy(同上)→ modules/<m>/<rel>.template;原文件保留为产物;
                   登记 kind=scaffold。
