...
| L6 | T 是普通文件 / 目录 | CONFLICT(普通文件:`--force` → BackupReplace;目录/特殊文件:`--force` 也拒绝,提示手工移走,ADR-29;建议 `dot add` 收编) |
...
- **单实例锁(ADR-19)**:mutation 命令对 `~/.local/state/dot/lock` 取 flock,失败即
  报错;`update` 自 `git pull` 起持锁。**进程内只获取一次**:顶层命令取锁,init/update
  的内部编排调用假定已持锁的 `applyLocked()`,引擎入口不自行取锁(flock 对同进程新 fd
  会阻塞,双重获取即自锁)。`diff`/`status`/`doctor` 只读不取锁——state 经原子 rename
  写入,只读方看到完整的旧版或新版。
...
7. 仓库侧预检(任何 mutation 之前;多路径输入先全部预检,任一失败则全部不执行):
   - 计算各输入的仓库目标路径与落地 target,复用 §5 的唯一性/后缀碰撞校验
     (输入之间、输入与既有 desired 之间:foo 与 foo.tmpl 去后缀同 target 即拒绝);
   - 仓库目标已存在(含后缀变体)→ 拒绝,绝不覆盖——state 丢失时仓库文件是唯一真相;
   - 记录各原文件的 hash 与 mode 快照。
8. 执行(按模式分支;仓库副本一律 O_EXCL 排他创建,绝不截断既有文件):
   默认(link):   copy → modules/<m>/<rel>(保留 mode,含可执行位);校验副本 hash;
                   目标同目录创建临时 symlink(.dot-tmp-*);**rename 前按预检快照
                   复核原文件 hash/mode**——失配(应用程序刚写入了新内容)→ 删除
                   临时链与仓库副本、保留原文件退出;复核通过 → 原子 rename 覆盖
                   原文件;登记 kind=symlink + link_dest。复核后的微秒竞态为已接受
                   剩余风险(01 §4)。
   --template:    [M2] copy → modules/<m>/<rel>.tmpl;原文件留在原位作为产物;
