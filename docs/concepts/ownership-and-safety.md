...
数据”。因此，一些看似保守的 `skip` 是产品语义，而不是需要绕过的障碍。
...
`status` 与 dry-run 是无锁只读入口；`apply` 获取 mutation lock 后再跑同一条循环，避免把
best-effort 的锁前观察当作执行依据。公开输出和退出码见 [CLI 规范](../spec/cli.md)，内部调用路径见
[架构概览](../architecture/overview.md)。

分析中常见的词可以这样阅读：

- **link / file / replace / remove**：会碰磁盘的行；
- **record**：叶子已经对了，只补账；
- **forget**：丢账，不动文件；
- **skip**：不会碰；出现任一 `skip` 时整批不写。

这些解释帮助阅读输出，不替代 [planning 规范](../spec/planning.md)中的完整判定。

## 失败后的恢复方式

`dot` 不承诺跨多个 target 的事务或自动 rollback。Apply 可能完成部分行后因外部 I/O、
...
重复收敛会从新的真实文件系统事实重新观察。更精确的执行顺序和完成边界见
