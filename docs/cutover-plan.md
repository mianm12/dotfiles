...
采用**先减法、后并行生长、一次切换**：阶段 A 砍掉非目标命令与特性（保持 `make check` 绿）；
阶段 B 在新包中按依赖顺序重建核心，不改写旧核心；B6 把 CLI 接线一次切换到新核心并整体删除
旧包；最后做全量验收与本机切换。

- 不采用一次性 nuke `internal/` 重建，也不采用对旧核心的分段原地改写：原地改写会迫使为 B4/B5
  就要删除的旧代码适配新语义（纯 shim 浪费），并行生长让每个检查点的绿都是真实的。
- 检查点绿的含义：旧实现与旧测试套件持续绿（阶段 A 删除的特性除外），已完成的新包各自带对齐
  §13 的测试独立变绿。
- 新核心包不得 import 旧核心包（`storage`、`lock`、`buildinfo` 等保留基础设施除外）；B6 切换
  前旧 CLI 行为不变，旧核心冻结、只许 bugfix。
...
表中阶段 B 涉及的"并入/坍缩重写/语义重写"均指在新包中并行重建目标语义；旧包及其测试保留到
B6 切换时整体删除。
...
### 阶段 B — 核心并行重建（新包生长，旧套件保持绿）

B1–B5 均在新包中实现并自带对齐 §13 的测试；各检查点写到的"删 X"，其旧代码删除统一推迟到
B6 切换时执行。
...
**B6 · CLI 切换：init / status / apply / remove**
- 非交互 `init`（§10.1，`--profile` 必填）、`apply`（§10.2 全量 + scoped + 加 extra）、
  **新建** `remove`（§10.3）、`status` 与 `--dry-run`（§10.4）、退出码（1 vs 2）。
- **切换与删除**：CLI 接线整体切到新核心；删除旧核心包（paths 的 control_*/name_semantics、
  state v1、旧 planner/apply/executor/runtime 等）及其测试；`make check` 全绿。
...
**C2 · 本机切换**（真实数据，唯一无测试保护的一步）
- 归档（移动而非删除）旧 state 与旧机器配置到带时间戳目录。
- 核对旧交互式 init 写出的机器配置与基线 §4.2 格式的差异；有额外语义先记录再丢弃。
- 用新 `dot init <repo> --profile ...` 重建并首次收敛。
- `dot status` 全部 converged；追加一次 `dot apply` 断言零 mutation。
...
- **B3 state v2 是断裂点**：按 §8 人工归档旧 v1 state，不做自动迁移（本机流程见 C2）；本机真实
  state 不参与开发（用合成 fixture）。
- **B6 是唯一的大步**：接线切换、旧包删除与全量绿在同一批完成，不允许新旧核心长期共存或混接。
...
- 本机完成 C2 切换：旧 state 与机器配置已归档，新 init 后 status 全 converged、重复 apply 零
  mutation。
