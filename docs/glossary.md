...
| state | 保存过去成功验证结果和 ownership/provenance 证据的私有账本 | [state and ownership](spec/state-and-ownership.md) |
| ownership | 允许 `dot` 更新或清理某个 link 所需的可验证证据，不是对任意路径的声明 | [state and ownership](spec/state-and-ownership.md) |
| provenance | Local 曾由哪个 example 初始化的记录；不授予覆盖或删除 local 的权限 | [state and ownership](spec/state-and-ownership.md) |
| actual filesystem | Analysis 时观察到的 target、ancestor 和 symlink 事实 | [planning](spec/planning.md) |
| analysis | 从 repository、selection、state 与文件系统生成只读 Report/Plan 的过程 | [CLI](spec/cli.md)、[planning](spec/planning.md) |
| plan | 对当前完整 selection 计算出的 transitions/actions 与 problems | [planning](spec/planning.md) |
| convergence | 让全部 effective modules 与历史 stale ownership 回到一致状态的完整过程 | [CLI](spec/cli.md) |
| action | Plan 中明确可执行或可展示的变化，例如 create、update、prune、forget | [planning](spec/planning.md) |
| problem | 阻止安全规划或 mutation 的客观事实 | [planning](spec/planning.md)、[CLI](spec/cli.md) |
| dry-run | 与 status 共用只读 analysis、展示当前 plan 而不执行 mutation 的 apply 模式 | [CLI](spec/cli.md) |
...
| prune | 在 ownership 与当前事实仍满足规则时删除 stale link | [planning](spec/planning.md) |
| forget | 放弃旧 ownership/provenance 证据而不删除不能安全修改的数据 | [planning](spec/planning.md) |
