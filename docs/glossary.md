...
| plan | 对当前完整 selection 一次计算出的有序 semantic actions 与 structured issues；可执行时还持有 next ownership state | [planning](spec/planning.md) |
...
| issue | Plan 中带 recovery 的 warning 或 blocker；只有 blocker 阻止执行 | [planning](spec/planning.md)、[CLI](spec/cli.md) |
