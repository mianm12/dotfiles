...
- [x] 2026-07-22：完成 Milestone 3 apply CLI stdio、hook outcome 投影与端到端验收；覆盖
  stdin/stdout/stderr、partial scope、失败退出、`--no-prune`/确认拒绝、幂等与 profile 恢复。
- [x] 2026-07-22：完成窄测、20 次重复、race、darwin/linux amd64 编译、branch diff check 与
  隔离 `make check`；更新 review handoff，保持计划 active 等待未参与实现者完整复核。
- [x] 2026-07-22：处理 Round 1 唯一有效 P2：runtime failed/deferred hook 行恢复规范闭集
  verb `run-hook`，仅以 `execution-failed` / `earlier-hook-failed` reason 表达 outcome；相关 CLI
  tests、CLI race、branch diff check 与隔离 `make check` 通过。
- [x] 2026-07-22：Round 2 未参与实现者完成全量复核，结论 GO、无 P0–P3 finding；main 仍为
  精确基线 `1df57addac93c48bc1497f1be15aa182a3730ce6`，freshness 无需同步，计划完成收口。
...
- Observation: Round 1 复核发现 runtime projection 曾把 outcome 拼进 verb，形成规范未允许的
  `run-hook (failed)` / `run-hook (deferred)`。
  Evidence: `docs/04-cli-spec.md` §5 将计划行 verb 封闭为包含 `run-hook`、但不包含上述两个派生
  verb；修复后的 plan 与真实 apply 测试都精确断言 `run-hook  <key>  (<outcome-reason>)`。
...
- Decision: hook 子进程直接使用 Cobra command 的实际 I/O writer/reader；dot 的 Result 摘要只在
  runner 返回后投影，不捕获或重放 hook 输出。
  Rationale: 这样 production `os.Stdin`/`os.Stdout`/`os.Stderr` 与测试注入走同一条路径，外部
  输出可实时出现，且 dot 自身仍只从已验证 Result 显示 succeeded/failed/deferred/skipped。
  Date: 2026-07-22

- Decision: hook runtime outcome 不扩展 CLI verb 集；failed/deferred 只进入既有 reason 字段。
  Rationale: `docs/04-cli-spec.md` §5 的 verb 是公开闭集，outcome 已有独立 reason 可以无歧义表达，
  无需修改规范迁就实现。
  Date: 2026-07-22

## Outcomes and Handoff

状态：completed。实现、全部本地门禁与要求的独立复核均已完成。

Round 1：唯一有效 P2 已修复。runtime HookRun 无论 succeeded、failed 或 deferred 都投影为
`run-hook`；reason 分别保留 `succeeded`、`execution-failed`、`earlier-hook-failed`。修复后运行
targeted projection/apply 测试、CLI 全包、CLI race、diff check 与隔离 `make check`，均退出 0；
`golangci-lint` 仍为 0 issues，全仓 race/build/manifest check 通过。

交付 commits：

- `2367964 feat(executor): 执行并复核 run_once hook`
- `04e8be6 feat(apply): 串行执行 hook 并原子提交成功效果`
- `486494b feat(cli): 接入 apply hook 实时输入输出`
- `af40f7b fix(cli): 保持 hook 运行结果的规范 verb`

实现结果：M1 string-form hook 由同一 canonical plan 驱动，executor 在启动前重观测普通文件、
bytes 和 direct/sh；apply 在 file/prune 后串行运行 HookRun、零执行 HookSkip，失败停止后缀并把
成功 file/prune/hook 前缀合入一个 `state.ChangeSet`、一次 CommitState。CLI 显式传递实际
stdin/stdout/stderr，并从密封 Result 投影 succeeded/failed/deferred/skipped。README 的当前实现
状态已同步；未修改 manifest/state 持久格式、依赖、config、init 或 runtime `InitSession`。

本地证据（均退出 0）：

- `go test -count=20 ./internal/executor ./internal/apply ./internal/cli ./internal/state ./internal/planner`
- `go test -race ./internal/executor ./internal/apply ./internal/cli ./internal/state ./internal/planner`
- `GOOS=darwin GOARCH=amd64 go test -c ... ./internal/executor`
- `GOOS=linux GOARCH=amd64 go test -c ... ./internal/executor`
- `git diff 1df57addac93c48bc1497f1be15aa182a3730ce6...HEAD --check`
- 使用 `/private/tmp` 的 `GOCACHE`、`GOLANGCI_LINT_CACHE` 与 `BINARY` 运行 `make check`；
  `golangci-lint` 为 0 issues，全仓 race、build 与 manifest check 通过。

测试只使用绝对 synthetic HOME/repo/config/state/backup/XDG；hook 与 CLI 测试显式核对真实 HOME
sentinel 未变化。darwin/linux 命令是编译证据，不等于目标平台运行；remote macOS/Linux CI 与
真实 Linux 实机均未运行，此限制保留为明确的未验证证据边界。

独立复核：Round 1 唯一有效 P2 已由 `af40f7b` 修复并重跑门禁；Round 2 由未参与实现者复核完整
Goal diff，结论为 GO、无 P0–P3 finding。复核时 main 仍精确位于基线
`1df57addac93c48bc1497f1be15aa182a3730ce6`，不存在需要同步的 freshness 漂移。计划现按
`.agent/plans/README.md` 迁入 completed，后续由主线程决定 FF-only 集成与主线验收。
