...
- [x] 2026-07-19：以 `bdedacd` 提交纯计划 reopen；先增加顺序、并发和失败重试回归，确认原
  `Ownership`/`Guard` 副本可重复消费引用。以 `b75b46a` 将 backend、路径、引用计数集中到共享
  owner state，每个 root/guard 使用共享 release token；lock 50 次、lock/runtime race、窄 lint
  和完整 `make check BINARY=/private/tmp/dot-handle-copy-lock-check` 通过。
- [x] 2026-07-19：先增加 mutation/init session 顺序与并发副本回归，确认原实现可重复加载并
  分叉 state 提交额度。以 `eb0e978` 将 lease、可信 context、加载 capability 和提交状态集中到
  私有共享 core；同一 `LoadedMutation` 的副本共享成功提交额度，Store 失败仍可由任一副本
  重试。runtime/lock/state 50 次、相关 race 和完整
  `make check BINARY=/private/tmp/dot-handle-copy-runtime-check` 通过。
- [x] 2026-07-19：三位未参与实现的只读 reviewer 对 `bf530d2ed4ce...88aa7e9` 完整分支
  复审均给出 GO，无 P0–P3 finding；分别确认共享 ownership token、runtime session core、失败
  重试、nested 锁序、规范数据流、Go API、测试和双平台构建风险，两个原 finding 全部关闭。
- [x] 2026-07-19：最终运行 runtime/lock/state 50 次重复、相关 race、全仓 `make check`、
  `go mod tidy -diff` 和 `git diff bf530d2ed4ce...HEAD --check`；当前 Darwin arm64 原生门禁、
  Darwin/Linux amd64 runtime/lock 测试二进制与 CLI 交叉编译均退出 0。本地验收通过，远端
  macOS/Linux CI 待验收。
...
| ownership/guard 值副本不能重复释放同一引用 | copy/concurrent/cross-process lock tests | 已通过 |
| mutation/init session 值副本共享阶段和提交额度 | copy/concurrent/state-preservation tests | 已通过 |
...
- `b75b46a` 将 OS lock backend、路径与引用计数集中到共享 owner state，并让 root/guard 的全部
  值副本共享一次性 logical reference token；Unlock 失败不消费 token，任一副本仍可重试。
- `eb0e978` 将 mutation/init 的 lease、加载阶段和提交额度集中到共享 session core；
  `LoadedMutation` 副本共享同一 capability，Store 失败可重试而成功发布只能发生一次。
...
首次收口后的合并前审查复现了 handle 值复制会分叉引用和提交状态；Milestone 7–9 已用共享
token/core 根本修复并通过三路完整复审。顺序、并发、跨进程与失败重试测试证明副本不能提前
Unlock、重复加载或二次发布 state，两个 finding 均已关闭且无 unresolved blocker。本地验收
通过；远端 macOS/Linux CI、Linux 实机运行和其他 macOS 文件系统格式仍待验收，不声称这些
平台证据已经通过。
