...
- [x] 2026-07-22：最终三路完整 Checkpoint Acceptance 对 clean `main@711bda8` 全部 GO，
  无 P0–P3；主线程完整 diff check、隔离 `make check`、四包五次重复和 Darwin/Linux amd64
  add/CLI test-binary 交叉编译通过。
- [x] 2026-07-22：最终 main 以 merge commit `1e91cfe` 非重写同步进 coordinator，无冲突；
  coordinator worktree clean，进入纯计划 closure 与 main FF-only 集成。
- [x] coordinator ExecPlan 终态更新并准备纯计划 closure；closure 后仅剩主 agent 执行 main
  FF-only 集成与终态门禁。
...
| 必须成立的性质 | 主要证据 | 状态 |
|---|---|---|
| 输入类型、module inference、`-m`、profile、完整碰撞 | add/manifest/path 单元与真实 FS 测试 | 通过 |
| manifest/Git ignore、local/global exclude、Git unavailable | 隔离真实 Git 集成与 injected runner | 通过 |
| 全输入预检、dry-run、M1 template 零写入 | runtime/add/CLI 文件系统测试 | 通过 |
| source variants、publication、等价续跑与 no-clobber | add publication 故障注入 | 通过 |
| link 提交点两侧、state failure、hard-link 隔离 | add runner + apply recovery | 通过 |
| scaffold render/mode、target snapshot、state 提交点 | add runner + apply recovery | 通过 |
| 多输入部分成功与已提交项不回滚 | add runner/CLI 集成 | 通过 |
| add 后 apply 收敛并立即重跑无 mutation/adopt | 全隔离 E2E | 通过 |
| macOS/Linux | 本地 Darwin + Darwin/Linux amd64 交叉编译 + 远端 CI | 本地通过；远端待验收 |
...
- Decision: 用户明确授权在 acceptance fix round 3 后继续第四轮；后续每个新发现仍以独立 active
  ExecPlan、semantic fix、完整 review、freshness 与 FF-only 集成处理。
  Rationale: 保持原 review 上限的停止语义与历史不改写，同时在新授权内闭合 module 恢复诊断和
  exit-code identity 的相邻契约。
  Date: 2026-07-22

## Outcomes and Handoff

本地 Checkpoint 已验收收口。四个 Milestone 严格按
`add-preflight → add-link → add-scaffold → add-cli` 依赖顺序，以独立 branch、active/completed
ExecPlan、semantic commits、完整 review、freshness 与 FF-only main 集成完成；closure 分别为
`669ea06`、`cff70b6`、`9206981`、`0ddb42a`。所有 worker worktree 均在 clean、已合入后无 force
移除，branches 保留。

首次 Checkpoint Acceptance 触发 `fix/m1-add-acceptance`。该分支及三个后续独立 ExecPlan 依次
闭合 module/source 候选诊断、确切 profile TOML 行、dotted key、profile/OS/target 八格恢复、
strict target-mapping typed error、`1 > 3` 和 explicit/inferred source mapping identity；closures
为 `15cdfca`、`d8fcf2a`、`014052c`、`711bda8`。最终退出码矩阵是：仅未显式选择的 zero/multiple
candidate 以 `ErrModuleAmbiguous` exit 3；所有 explicit invalid/missing/nonmembership、OS/组合
activation、target mapping、source zero/multiple 与其他 strict/runtime error exit 1；dry-run 2，
成功 mutation 0。恢复指引保持稳定，分类不解析文本。

最终三名未参与实现的 reviewer 对 `5d176497...main@711bda8` 分别复核规范/公开行为、安全/恢复
边界、工程/测试/双平台，全部 GO，无 P0–P3。主线程再次运行完整 checkpoint diff check、隔离
`make check`、manifest/add/CLI/apply 五次重复及 Darwin/Linux amd64 add/CLI test-binary 交叉编译，
全部通过；mutation 测试只使用合成 HOME/repo/config/state/backup，并断言真实 HOME sentinel
不变。真实 Linux 主机与远端 macOS/Linux CI 未运行，因此准确状态为“本地验收通过、远端待验收”。

最终 main 已以 `1e91cfe` 非重写同步进 coordinator；本计划 closure commit 后，主 agent 只需把
`chore/m1-cp6-orchestration` FF-only 合入 main、运行终态门禁并移除 clean coordinator worktree。
