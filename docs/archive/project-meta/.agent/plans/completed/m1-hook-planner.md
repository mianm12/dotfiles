...
- [x] 2026-07-19：以 `1290501 docs(planner): 建立 hook planner 执行计划` 提交 active
  ExecPlan 起点。
- [x] 2026-07-19：测试先行建立执行分类、versioned fingerprint 与自包含单 hook action，以
  `8aa2a81 feat(planner): 建立 hook 指纹与动作` 提交。
- [x] 2026-07-19：测试先行组合 strict state、顺序、partial scope、历史保留与 fail-fast 完整
  plan，以 `a8c4d35 feat(planner): 组合 scoped run_once 计划` 提交。
- [x] 2026-07-19：通过窄测、20 次、race、darwin/linux amd64 交叉编译、branch diff check 与
  `make check`；计划保持 active，等待独立复核。
- [x] 2026-07-19：未参与实现的 reviewer 完整复核 `1a7e0fc...dfb5b45`，结论 GO、无
  P0–P3 finding；复核额外运行 Hook 测试 20 次与 diff check，worktree 保持 clean。
- [x] 2026-07-19：主线程复验 Hook 测试 20 次、完整 diff check 与 `make check` 全部通过，
  迁移本计划至 completed，准备以纯计划提交收口。
...
| 性质 | 证据 | 状态 |
|---|---|---|
| regular/executable 分类与 invocation | `TestPlanHook_BuildsSelfContainedActionWithoutExecution` 等 filesystem tests | 通过 |
| versioned 无歧义指纹及分类变化 | fingerprint golden/table tests | 通过 |
| missing/same/changed run_once | strict state plan tests | 通过 |
| 模块/声明顺序与 partial scope | manifest integration tests | 通过 |
| 历史不清理、conflict 不门控、fail-fast 零 partial | plan boundary tests；Hook planner API 不接收 file conflict | 通过 |
| 当前平台完整门禁 | darwin/arm64 `make check BINARY=/private/tmp/dot-cp3-hook-planner-check/dot` | 通过 |
| 双平台编译证据 | darwin/amd64、linux/amd64 `go test -c ./internal/planner` | 通过（未执行二进制） |
| 独立完整复核 | reviewer 审查 `1a7e0fc...dfb5b45` 并重复 Hook 测试 20 次 | GO，无 P0–P3 finding |
...
- `state.Loaded` 只有 missing 与携带严格 `Snapshot` 的 loaded 两种可消费状态；零值必须在读取
  script 前 fail closed，避免无效 history 与脚本错误竞争成为表面错误。
- strict state 可能包含当前 profile/scope 已不存在的 `run_once` key；planner 只按
  `ScopedProfile.Hooks()` 生成动作，因此天然保留这些历史记录，不需要 cleanup action。
...
- Decision: `PlanHooks` 在任何脚本 I/O 前验证 `state.Loaded`，但指纹只包含固定版本、执行分类和
  script bytes。
  Rationale: 无效 state 不能形成可信计划；profile、HOME、XDG、DOT_* 与 state 时间仅属于执行
  上下文，不应导致脚本内容未变时重复运行。
  Date: 2026-07-19

- Decision: 相同指纹形成 `skip/preserve`，缺失或变化形成 `run-hook`，并分别显式保存成功
  upsert 与失败 preserve；不为历史 key 创建删除动作。
  Rationale: 这直接表达 at-least-once 与“成功后更新、失败保留最后成功记录”的恢复边界，并让
  future executor 不需要重新推断 state 语义。
  Date: 2026-07-19

## Outcomes and Handoff

实现与独立复核均已完成。相对 current main `1a7e0fc` 的 branch 包含 freshness merge commit
`1f11fcd`、计划 commit `1290501`、两个行为 commit `8aa2a81`、`a8c4d35`，以及交接证据 commit
`dfb5b45`；共享 model、decision/prune、manifest 接缝未修改。独立 reviewer 结论为 GO、无
P0–P3 finding，主线程随后重复 Hook 测试 20 次、完整 diff check 与 `make check`，全部通过。
...
以上均通过。双平台只完成编译，远端 macOS/Linux CI 尚未运行；本地验收通过、远端待验收。
未验证项是 future executor 的实际 hook 执行、成功后 state 持久化和失败重试，因为它们明确不在
本 Milestone scope 内。
