...
- [x] 2026-07-20：修复提交 `ae2a9e1` 后，窄测试、完整 diff check 与隔离 cache 的
  `make check` 重新通过。
- [x] 2026-07-20：第二轮未参与实现的 reviewer 完整复审结论为 GO、无 findings；main 仍精确
  等于有效 base `f7da6a6`，最终 freshness 窄测试、diff check 与 `make check` 全部通过。
...
- review fix `ae2a9e1` 后再次运行上述窄测试、diff check 与完整 `make check`：通过；同步顺序和
  中途失败不返回成功由故障注入测试固定。
- 第二轮完整复审：GO，无 findings。最终 freshness 时 `main` 与有效 base 均为 `f7da6a6`；
  `go test ./internal/backup -count=1`、完整 diff check 与隔离 cache 的 `make check` 再次通过。
