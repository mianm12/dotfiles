...
placement 或 cleanup step；已由 `select remove` 移出 extra selection 的目标不再
...
step，放弃对应 state ownership，不阻塞本轮其余收敛。

该宽容规则仅适用于 stale placement——`dot` 对它唯一想做的 step 是删除，放弃删除不触碰任何
...
时永不删除；若 state 有记录则生成带原因的 forget step，只忘记 provenance。Remove/prune
永不删除 local。
