...
- 人类输出走 stdout;错误、warning 与 compatibility notice 走 stderr;为脚本消费预留
  `--json` [M3]。
- stdout 与 stderr 是独立 stream,不提供跨 stream 事务或回滚:任一写入失败都按运行错误退出 1,
  此前已成功写出的内容不撤回。命令章节若明确规定错误不得与可信 verdict 并存(如
  `status` 的 `Clean.`),该命令必须在发布 verdict 前确认其必要输出成功;`doctor`、`version`
  等诊断型例外仍可保留已经安全给出的信息。
...
- dot 自身的 context、模块、文件、prune 与 verdict 摘要按规范稳定排序,便于人工比较和脚本
  验证。`apply`/`update`/嵌套 `init` 执行 hook 时,hook 继承调用命令的 stdin/stdout/stderr 并
  实时透传；脚本输出可能与 dot 摘要及另一 stream 交错,不属于确定性输出契约,也不得被 dot
  buffer、截断、解析或重排。
