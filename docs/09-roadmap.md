...
*说明:M1 相比最初构想大了一圈(owned/收养/不变量/锁/收敛门控/fail-closed),原因
一致——这些要么属于**持久化契约**(state 字段与语义、run_once 键格式、requires 行为),
要么属于**安全底线**(误删/误覆盖/丢配置防护)。契约后补有兼容债,底线后补等于先裸奔;
而 watch、edit、from_env 之类后补皆无痛,留在后面。*
